...
- **Per-asset licensing you can inspect.** Every record carries an SPDX identifier, a license URL, an explicit `requiresAttribution` flag, a `sha256` over the bytes, and its source/provider provenance. Only `trusted-default` records (verified review plus a commercial-use-compatible license plus a checksum that matches) are auto-selected; anything attribution-required or unverified is browseable but needs an explicit human decision. The asset layer committed in this repo records **80 assets as `CC0-1.0`**; the two non-CC0 entries (one `CC-BY-4.0`, one `LicenseRef-Unknown`) are test fixtures under `asset-layer/catalog-fixtures/`. That describes what this repo records, not an audit of what the remote serves.
- **You name the endpoint.** No deployment host is baked into Loombridge. Point the CLI at a catalog with `--catalog-api <baseUrl>` (a search API) or `--catalog <url>` (a shard directory or a `.jsonl` file); with neither set, the catalog layer refuses with a named error rather than reaching for someone's server. `LOOMBRIDGE_ASSET_CATALOG_URL` is the environment variable that names the same URL: the Unity Asset Browser reads it directly, and the CLI verbs take it through the flag (`--catalog "$LOOMBRIDGE_ASSET_CATALOG_URL"`). The current base URL is published alongside the asset store at **https://assetstore.loomtide.ai/**.

Browse candidates in-editor via **Window → Loombridge → Asset Browser**, or from the CLI: [`Docs/Assets/PublicCatalogQuickstart.md`](Docs/Assets/PublicCatalogQuickstart.md).
