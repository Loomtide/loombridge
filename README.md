...
Two doors into one bridge: building needs an agent, verifying deliberately does not.
...
flowchart TB
  ...
  agent --> build
  build --> mcp["MCP server<br/>typed op registry"]
  verify --> cli["loombridge CLI<br/>owns gates, exits, verdicts"]
  feel --> cli

  mcp --> bridge
  cli --> bridge
  bridge["Unity bridge (C#)<br/>main-thread op handlers"] --> unity["Unity Editor<br/>your game"]
  cli -.writes.-> state[".loombridge/<br/>state · traces · reports"]
```
