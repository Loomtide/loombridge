...
(`mcp-server/src/loomtide/genre-packs/platformer-2d/profiles/*.profile.json`): a named set of measurable
...
(`mcp-server/src/loomtide/genre-packs/platformer-2d/solve-params.ts`) — the build-side counterpart to the
...
`scripts/sync-loomtide-artifacts.mjs` — do not edit by hand; edit the profile and regenerate):

<!-- BEGIN GENERATED: starting-params (scripts/sync-loomtide-artifacts.mjs) -->
