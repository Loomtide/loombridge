...
The prepare report also includes checksum, cache status, provider diagnostics, selected-vs-rejected counts, and accepted import arguments. Generate the attribution artifact with `npm run asset:platformer:attribution` after `npm run asset:platformer:prepare`.
