...
The output is a strict JSON rubric (schema: `mcp-server/src/verification/vlm-review.schema.json`), saved as ONE consolidated review at the verify root (`.loombridge/verify/vlm-review.json`) that references every state's frames by their state-relative path, and merged by `loombridge verify --vlm` under the advisory `reviewFindings` key (see the flow in §"Merge ONE consolidated review" below).
