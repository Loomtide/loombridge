...
(`unity-projects/shooter-combat-dogfood`, scene `Assets/Scenes/ShooterCombatDogfood.unity`) — a real shooter build,
...
npm run build
cd ..
node demo-bundles/generic-build-follow-on/2d-shooter-promoted-vertical/generate.mjs
cd mcp-server
node --test dist/__tests__/genre-contract-promotion.test.js dist/__tests__/generic-build-follow-on.test.js
```

The live shooter-combat-loop capture itself is re-runnable from `unity-projects/shooter-combat-dogfood`: open the
