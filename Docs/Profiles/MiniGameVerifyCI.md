...
This page is everything you need to run `loomtide verify --minigame` on a CI runner: how to install
`loomtide` without the agent surface, how to confirm which build you're running, the one CI command, how to
upload the report, and what the exit codes mean to a pipeline.

---

## 1. Install `loomtide` on a runner (no `~/.claude`)

The local install script (`scripts/loomtide-install-locally.sh`) sets up the *agent* surface — slash
commands and skills under `~/.claude`, a frozen runtime under `~/.loomtide`. **CI does not want any of that.**
Install just the CLI, two ways:

**A. Build from a pinned checkout (recommended for CI).** Reproducible and ref-pinned:

```bash
git clone https://github.com/Loomtide/loombridge loomtide
cd loomtide && git checkout <PINNED_SHA_OR_TAG>
cd mcp-server && npm ci && npm run build
# invoke directly — no global install, no ~/.claude:
node "$PWD/dist/cli.js" --version
```

Wrap it in a shell alias / step so the rest of the pipeline can call `loomtide`:

```bash
echo "alias loomtide='node $PWD/dist/cli.js'"     # or add a wrapper to $PATH
```
...
```bash
cd mcp-server && npm ci && npm pack          # -> loomtide-cli-<version>.tgz
                                             #    (prepack builds dist + packs the bundled bridge tarball)
npm i -g ./loomtide-cli-<version>.tgz        # installs the `loomtide` bin + pulls runtime deps
loomtide --version
```

(Or `npm link` from `mcp-server` on a dev box — same `loomtide` bin, no install.) The installed `loomtide`
runs the same code path as the packed tarball; `loomtide --version` confirms the build either way.
...
## 2. Confirm the build — `loomtide --version` (stale-runtime guard)

```bash
$ loomtide --version
loomtide 0.1.0 (a1b2c3d, built 2026-06-07T16:01:17.458Z)
```
...
built without git). This is the guard against the most common footgun: a `loomtide` that execs a **frozen,
...
requires driving the game in the Unity editor through the Loomtide bridge: `loomtide minigame capture`
...
```bash
WORKSPACE="${LOOMTIDE_WORKSPACE:-$HOME/.loomtide/projects/$GAME_ID}"
...
- It is **read-only**; it never mutates your game. It writes three files next to the report path
  (normally `~/.loomtide/projects/<id>/reports/minigame-verification.{json,html,md}` via
...
| **1** | `NOT READY` | A **game/asset defect** (control off-screen / too small / clipped, flow didn't reach its reward) **or** a **baseline drift**. | The **game** team — fix it; or, if the visual change was intended, re-approve the baseline (`loomtide minigame baseline approve`). |
...
```bash
set +e
loomtide verify --minigame --contract "$CONTRACT" --captures "$CAPTURES" --strict
CODE=$?
set -e
case "$CODE" in
  0) echo "READY — ship." ;;
