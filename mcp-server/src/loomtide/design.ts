/**
 * `loomtide design` — the deterministic half of the Design Target Phase (plan §3c).
 *
 ...
 * is the agent layer's job (`commands/loomtide/plan.md`), never the CLI's.
 */
...
// At runtime this file is dist/loomtide/design.js. The canonical (product-owned)
// schema for the artifacts this module writes lives next to the source under
// `src/loomtide/schemas/` — v0.6 explicitly settled the location decision: the
...
/** `.loomtide/design/design-target.json` — the frozen target's provenance. */
export interface DesignTargetMeta {
  ...
  /** Hero-shot image filename, relative to `.loomtide/design/`. */
  heroShot: string;
  /** Optional editable source filename, relative to `.loomtide/design/`. */
...
export function designPaths(paths: LoomtidePaths): DesignPaths {
  return {
...
export async function designStatus(paths: LoomtidePaths): Promise<DesignStatusReport> {
...
/** Ingest + freeze a hero shot into `.loomtide/design/` and record provenance. */
...
export async function approveDesignTarget(args: { root: string; note?: string }): Promise<DesignTargetMeta> {
  ...
  const meta = await readMeta(dp);
  if (!meta) {
    throw new Error("No design target to approve — run `loomtide design set --image <path>` first.");
  }
  const hash = await sha256OrNull(dp.heroPng);
...
// ─────────────────────────────────────────────────────────────────────────────
// CLI (`loomtide design <status|set|approve>`) — a non-headline helper the plan
...
function printUsage(): void {
  console.log(
    [
      "Usage: loomtide design <status|set|approve> [options]",
      "",
      "Manage the Design Target (annotated hero shot) in .loomtide/design/.",
      "",
...
/** CLI entry: `loomtide design <action> [flags]`. */
export async function run(args: string[]): Promise<number> {
  ...
  try {
    ...
    console.error(`[loomtide design] ${error instanceof Error ? error.message : String(error)}`);
    printUsage();
    return 2;
  }
  ...
  try {
    switch (action) {
      case "status": {
        const report = await designStatus(loomtidePaths(flags.root));
        ...
        const compRefNote =
          report.status === "approved" && report.kind === "composition-reference"
            ...
            : "";
        console.error(
          `[loomtide design] status=${report.status}${frozen} kind=${report.kind} png=${report.hasPng} html=${report.hasHtml}` +
      ...
      case "set": {
        ...
        console.error(`[loomtide design] set hero shot (status=${meta.status}, kind=${meta.kind ?? DEFAULT_DESIGN_TARGET_KIND}, mode=${meta.mode}).`);
        return 0;
      }
      case "approve": {
        const meta = await approveDesignTarget({ root: flags.root, note: flags.note });
        console.error(`[loomtide design] approved (frozen sha256=${meta.pngSha256.slice(0, 12)}…).`);
        return 0;
      }
      default:
        console.error(`[loomtide design] unknown action "${action}".`);
        printUsage();
        return 2;
    }
  } catch (error) {
