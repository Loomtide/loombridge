 ...
 * S6b+ wires real per-project `.loomtide/games/<id>.json` contracts to capture.
 */
...
// __dirname is dist/loomtide/minigame-profiles at runtime; the JSON lives in the
// source tree (not copied by tsc), so resolve to the package root and read src/.
const PKG_ROOT = path.resolve(__dirname, "..", "..", "..");
const GAMES_DIR = path.join(PKG_ROOT, "src", "loomtide", "minigame-profiles", "games");
