...
function spawnCli(argv: string[], env: NodeJS.ProcessEnv): number {
  ...
  if (res.error) {
    console.error(`[loomtide minigame] check: could not spawn step: ${res.error.message}`);
    return 1;
  }
  return res.status ?? 1;
}
...
export async function runSceneAgnosticCheckCli(id: string, root: string = process.cwd()): Promise<number> {
  ...
  const deps: SceneAgnosticCheckDeps = {
    ...
    writeContract: async (c) => {
      ...
      if (applied.length > 0) {
        console.log(`[loomtide minigame] check: re-applied persisted overrides from ${path.basename(overridesPath(workspace))} (${applied.join(", ")}).`);
      }
      if (dropped.length > 0) {
        console.error(`[loomtide minigame] check: ignored invalid override field(s) in ${path.basename(overridesPath(workspace))}: ${dropped.join(", ")} — fix or remove them.`);
      }
