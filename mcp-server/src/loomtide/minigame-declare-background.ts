/**
 * `loomtide minigame declare-background` — close the report→contract loop.
 *
...
function printUsage(): void {
  const lines = [
    "loomtide minigame declare-background — declare decorative background so the sweep stops flagging it",
    "",
    "Usage:",
    "  loomtide minigame declare-background [--id <kebab> | --contract <path>] <scene-path>...",
    "  loomtide minigame declare-background --id <kebab> --from-report   # use the report's suggestions",
    "",
    ...
    "",
    "Options:",
    "  --id <kebab>       Mini-game id; resolves ~/.loomtide/projects/<id> and finds the contract.",
...
/** The latest verify JSON report for a workspace — checks both the standalone (`.loomtide/reports`)
...
async function loadReport(workspace: string): Promise<MinigameVerifyReport | null> {
  const candidates = [
    path.join(workspace, ".loomtide", "reports", "minigame-verification.json"),
...
export async function runDeclareBackground(argv: string[], root: string = process.cwd()): Promise<number> {
  ...
  console.log(`   loomtide verify --minigame --contract ${tildify(contractPath)} --captures ${tildify(captures)} --output ${tildify(reportOut)} --strict`);
  return 0;
}
