...
import { readFrameImage, type DecodedImage } from "./minigame-gates/frame-facts.js";
import type { MinigameCaptureObject, MinigameRect, MinigameViewport } from "./minigame-gates/types.js";
...
export async function writeBaselineBundle(args: {
  ...
  if (skipped.length > 0) {
    console.error(
      `[loomtide minigame] baseline: skipped ${skipped.length} uncaptured state(s) (outcome-gated or no capture present): ${skipped.join(", ")}.`,
    );
  }
...
export async function compareToBaseline(
  ...
  if (!manifest) {
    return {
      ...base,
      present: false,
      note: `No approved baseline at '${refDir}' — declared but not yet approved (run \`loomtide minigame baseline approve\`). Baseline regression not enforced.`,
      states: [],
      regressions: [],
      incompleteStates: [],
    };
