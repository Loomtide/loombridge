/**
 * Pure core of `loomtide minigame capture` (no bridge, fully unit-testable):
 *
...
import type { MinigameCaptureObject } from "./minigame-gates/types.js";
import type { FlowActuation, FlowEvidence } from "./minigame-gates/flow-evidence.js";
import type { MinigameContract, MinigameReachedCondition } from "./minigame-profiles/types.js";
...
export function toCaptureState(
  ...
  return {
    state: stateId,
    source: "loomtide minigame capture (full-scene dump)",
    viewport: {
      width,
      height,
...
export function toResponseDoc(
  ...
  return {
    source: "loomtide minigame capture (input-response before/after)",
    tap: stimulusTarget,
