/**
 * `loomtide doneness` — the §3a supervisor freshness predicate.
 *
...
export interface VerdictLike {
  ...
  runId?: string | null;
  producedAt?: string;
  /** Frozen Design Target metadata (plan §3c); present on `loomtide verify` output. */
   ...
   * partial status. ALWAYS emitted by `loomtide verify`. When the on-disk contract
...
export const COMPOSITION_REFERENCE_REFUSAL =
  "design target is an APPROVED `composition-reference` (style/composition guide for scene assembly only) — it is NOT a frozen hero shot and can never certify doneness. Assemble the 3D scene, capture a real Unity frame, then `loomtide design set --image <frame> --kind rendered-unity-frame --approve` and re-run (the composition → assemble → capture → freeze → fidelity flow, plan §3c).";
...
export function diskTruthDesignTargetRefusals(
  ...
  if (design.status !== "approved") {
    if (stateDesignTarget === "approved" && verdict?.designTarget?.status !== "approved") {
      return [
        "STATE says Design Target is `approved` but the on-disk design target is missing/unapproved and the verdict carries no approved `designTarget` binding — cannot prove hero-shot fidelity was evaluated; restore the target or re-run `loomtide verify` against the approved target (plan §3c/§P0.1)",
      ];
    }
    return [];
  }
  ...
  if (!bound) {
    ...
    reasons.push(
      `an approved Design Target (\`rendered-unity-frame\`) is on disk but the verdict carries no matching approved \`designTarget\` bound to the frozen hero shot${sha} — hero-shot fidelity was not evaluated; run \`loomtide verify\` against the approved target so the verdict binds it (plan §3c/§P0.1)`,
    );
  }
 ...
 * `inputToSfxLatency` / `sfx-fatigue`) — proof `loomtide verify` actually GRADED the
...
export function sfxGateRefusals(
  ...
  if ("unreadable" in declared) {
    return [
      `contract unreadable — cannot determine whether SFX verification is declared (${declared.unreadable}); a malformed contract must not disarm a declared SFX gate; refusing (fix .loomtide/ACCEPTANCE.json — SFX-dogfood #7/D1)`,
    ];
  }
  ...
  if (!gates || typeof gates !== "object" || Array.isArray(gates)) {
    return [
      "contract declares SFX verification (verification.sfx.enabled) but the verdict graded no SFX gates (no usable `gates` block) — re-run `loomtide verify` so the SFX gates are graded (SFX-dogfood #7/D1)",
    ];
  }
  const reasons: string[] = [];
  for (const gate of SFX_GATE_NAMES) {
    if (typeof gates[gate] !== "string") {
      reasons.push(
        `contract declares SFX verification (verification.sfx.enabled) but the verdict carries no \`${gate}\` gate report — the SFX surface was not graded by this verify run; re-run \`loomtide verify\` (SFX-dogfood #7/D1)`,
      );
    }
  }
  return reasons;
...
export function evidenceClassRefusals(
  ...
  if (!Array.isArray(requiredEvidenceClasses)) {
    return [
      `contract unreadable — cannot determine required evidence classes (${requiredEvidenceClasses.unreadable}); a malformed contract must not disarm a declared evidence gate; refusing (fix .loomtide/ACCEPTANCE.json — dogfood §6/#7)`,
    ];
  }
  ...
  for (const cls of requiredEvidenceClasses) {
    ...
    if (!block) {
      reasons.push(
        `contract requires evidence class \`${cls}\` to be \`present\` but the verdict has NO \`evidenceClasses\` block — cannot prove the signal was gathered; refusing (re-run \`loomtide verify\` — dogfood §6/#7)`,
      );
      continue;
    }
    ...
    if (!rederived) {
      reasons.push(
        `contract requires evidence class \`${cls}\` and the verdict's evidenceClasses claims \`present\`, but the verdict carries no usable \`gates\` block to re-derive it from — a stored block cannot self-attest; refusing (re-run \`loomtide verify\` — dogfood §6/#7)`,
      );
      continue;
    }
...
export interface CaptureCheck {
  /** Path relative to `.loomtide/verify/` (the captureManifest entry). */
  path: string;
  exists: boolean;
  /**
   * True when this entry failed the safe-path check (absolute / `..` /
   * non-canonical) OR when its resolved absolute path escaped
   * `.loomtide/verify/`. Surfaced as a distinct hard failure — counting it
...
/**
 * Strict shape check for `.loomtide/verify/vlm-review.json`.
 *
...
async function checkSliceRollupHeroShotFidelity(paths: LoomtidePaths): Promise<string[]> {
...
async function checkSliceRollupAssetSourceFidelity(paths: LoomtidePaths): Promise<string[]> {
  const design = await designStatus(paths);
  if (design.status !== "approved") return [];
  ...
  try {
    ...
    if (!manifest) {
      return ["approved Design Target requires `.loomtide/ASSET_MANIFEST.json`; source fidelity cannot be certified without manifest bindings"];
    }
...
export function isFreshGreen(input: FreshGreenInput): FreshGreenResult {
  ...
  if (!state) {
    return { ok: false, reasons: ["no .loomtide/STATE.md (run `loomtide plan` first)"] };
  }
  ...
  if (!state.currentBuild) {
    reasons.push("no `currentBuild` in STATE — no build is in flight (run `loomtide build` first)");
  ...
  if (!verdict) {
    reasons.push("no verdict at .loomtide/reports/build-verdict.json");
  ...
  if (state.currentBuild && verdict) {
    if (!verdict.runId) {
      reasons.push(
        "verdict has no `runId` — likely produced before `loomtide build` minted `currentBuild` (stale)",
      );
 ...
 */
export async function isSliceDone(slice: SliceEntry, paths: LoomtidePaths): Promise<SliceDoneResult> {
  const reasons: string[] = [];
...
export interface DonenessArgs {
  root: string;
  /** Override the verdict path (default: `.loomtide/reports/build-verdict.json`). */
  verdictPath?: string;
}
 ...
 * bridge `loomtide_status`) consume the exact same predicate — `verifiedGreen`
...
export async function wholeGameDonenessReasons(
...
): Promise<{ reasons: string[]; state: LoomtideState | null; verdict: VerdictLike | null; manifestCount: number; artDeferred: boolean }> {
  const state = await readState(paths);
  ...
  const captures: CaptureCheck[] = await Promise.all(
    manifest.map(async (entry) => {
      ...
      // `.loomtide/verify/`. This catches edge cases (symlinks, future ifs).
      const candidate = path.resolve(paths.verifyInputs, entry);
...
/**
 * Read-only boolean: would `loomtide doneness` certify this project right now?
 * The SAME gate, reused — NOT a re-implementation — so the bridge `loomtide_status`
 ...
 */
export async function isProjectDone(paths: LoomtidePaths): Promise<boolean> {
...
export async function runDoneness(args: DonenessArgs): Promise<number> {
  ...
  // point at `loomtide plan`. (A project WITH a roadmap or state went through the
  ...
  if (!(await fileExists(paths.acceptance)) && !slicePlan) {
    ...
    if (!state) {
      ...
      console.error(
        `[loomtide doneness] NOT done — ${noContractRefusal(paths.acceptance, presence.capturePresentDirs)}`,
      );
      return 1;
    }
  }
  ...
  if (!(await acceptanceContractValid(paths.acceptance))) {
    const r =
      "ACCEPTANCE.json is missing or does not parse as a valid acceptance contract — doneness cannot certify against an unvalidatable contract (fix .loomtide/ACCEPTANCE.json or re-run `loomtide plan`; the same check gates `loomtide_status.verifiedGreen`)";
    if (!reasons.some((existing) => existing === r)) reasons.push(r);
  }
  ...
  if (reasons.length === 0) {
    ...
    console.error(
      `[loomtide doneness] OK — fresh + green (runId=${runId}, ${manifestCount} captures verified${fidelity}).`,
    );
    return 0;
  }
  console.error("[loomtide doneness] NOT done:");
  for (const r of reasons) console.error(`  - ${r}`);
  return 1;
}

async function runSliceDoneness(plan: SlicePlan, paths: LoomtidePaths): Promise<number> {
  const ev = await evaluateSliceDoneness(plan, paths);
  if (ev.emptyRoadmap) {
    console.error("[loomtide doneness] NOT done — SLICES.json has no slices (empty roadmap).");
    return 1;
  }

  console.error("[loomtide doneness] Slice roll-up:");
  ...
  if (ev.ok) {
    console.error(`[loomtide doneness] OK — ${plan.slices.length}/${plan.slices.length} slices approved + deterministic proofs fresh + hero-shot faithful + asset-source faithful.`);
    return 0;
  }
  console.error("[loomtide doneness] NOT done — slice roll-up refused.");
  return 1;
}
...
function printUsage(): void {
  console.log(
    [
      "Usage: loomtide doneness [options]",
      "",
      ...
      "                      (default: .loomtide/reports/build-verdict.json)",
...
export async function run(args: string[]): Promise<number> {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--root") root = path.resolve(args[(i += 1)] ?? root);
      ...
      console.error(`[loomtide doneness] unknown argument "${arg}".`);
      printUsage();
      return 2;
    }
  }
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    console.error(`[loomtide doneness] fatal: ${message}`);
    return 1;
  }
}
