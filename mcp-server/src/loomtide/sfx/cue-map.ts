...
// At runtime this compiles to dist/loomtide/sfx/cue-map.js, so the package root is
 ...
 * (`src/loomtide/genre-packs/<genre>/cue-map.json`). Pure path construction — does
...
export function cueMapPathForGenre(genreId: string): string | null {
  ...
  return path.join(PKG_ROOT, "src", "loomtide", "genre-packs", genreId, "cue-map.json");
}
