/**
 * `loomtide minigame setup` — guided first-time onboarding (a WRAPPER, not a new
 * engine).
 *
 * Power/CI users keep driving the proven low-level verbs directly:
 *
 *   loomtide minigame init
 *   loomtide trace record/replay
 *   loomtide verify --minigame
 *   loomtide minigame baseline approve|status
 *
 ...
 * for a mini-game lives FLAT under `~/.loomtide/projects/<id>/` — `traces/` holds the
 * recorded trace, `reports/` the verify + replay reports, `captures/` the capture pack,
 * `baseline/` the approved baselines. No nested `.loomtide/`, no separate replay root.
 */
...
/**
 * The default workspace path for an id: `~/.loomtide/projects/<id>`. Deliberately
 ...
 * the rest is resolved one step at a time by `loomtide minigame next`, so the guidance can
...
export function buildNextStepsChecklist(config: SetupConfig, _root: string): string[] {
  ...
  return [
    ...
    ...formatNextStep(step),
    "",
    `(Run \`loomtide minigame next --id ${config.id}\` anytime to see your next step.)`,
  ];
}
...
export async function runSetup(argv: string[], root: string = process.cwd()): Promise<number> {
  ...
  if ("error" in flags) {
    console.error(`[loomtide minigame] setup: ${flags.error}.`);
    printUsage();
    return 2;
  }
  ...
  if ("usage" in resolution) {
    console.error(`[loomtide minigame] setup: ${resolution.usage}.`);
...
export async function scaffoldWorkspace(
  ...
  try {
    ...
    console.error(`[loomtide minigame] setup: could not create workspace ${config.workspace}: ${message(error)}`);
    return 1;
  }
  ...
  if (exists && !force) {
    console.error(
      `[loomtide minigame] setup: ${display(root, contractPath)} already exists — refusing to overwrite. ` +
  ...
  try {
    ...
    console.error(`[loomtide minigame] setup: could not write contract ${contractPath}: ${message(error)}`);
    return 1;
  }
...
function printUsage(): void {
  console.error(
    [
      "Usage: loomtide minigame setup [options]",
      "",
      ...
      "  --workspace <dir>        Workspace dir (default: ~/.loomtide/projects/<id>; must be",
