...
import { computeStatusModel, developerNextAction, type LoomtideStatusModel } from "./status-model.js";
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--root") {
      ...
      if (!value || value.startsWith("--")) {
        console.error("[loomtide ask] --root requires a project directory.");
...
function currentSliceDetails(model: LoomtideStatusModel): string {
...
function nextLine(model: LoomtideStatusModel): string {
  return `Next: ${developerNextAction(model)}`;
}

function warningLines(model: LoomtideStatusModel): string[] {
...
function warningSummary(model: LoomtideStatusModel): string | null {
...
export function answerAsk(model: LoomtideStatusModel, question?: string): string {
...
export async function runAsk(args: AskArgs): Promise<number> {
  ...
  try {
    ...
    console.error(`[loomtide ask] ${message}`);
    return 2;
  }
}
...
function printUsage(): void {
  console.log(
    [
      "Usage: loomtide ask [question...] [options]",
      "",
      "Read-only explanation of the current Loomtide project state. Answers from",
      "local .loomtide/ state only; never builds, captures, verifies, plans, or approves.",
      "",
      "Examples:",
      "  loomtide ask where are we",
      "  loomtide ask what should I do next",
      "  loomtide ask why is approval needed",
      "",
      "Options:",
