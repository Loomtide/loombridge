/**
 * `build-stamp` — the single source of truth for "which build of loomtide is
 ...
 * install (`scripts/loomtide-install-locally.sh` copies `dist/` wholesale):
 ...
 *
 * This is exactly the data `loomtide --version` prints (see `cli.ts#printVersion`).
...
export interface BuildStamp {
  /** Always `"loomtide"` — the tool that produced the artifact. */
  tool: "loomtide";
 ...
 *    sits in `dist/loomtide/`, so the package root is two levels up:
 *    `../../package.json`.
...
/**
 * One-line human-readable form, matching `loomtide --version` style
 * (`loomtide 0.1.0 (86bb72b+dirty, built 2026-…)`). Used for the CLI report line
