/**
 * `loomtide doctor` — health check for the local install + a project's bridge
 ...
 * With `--project`: the install metadata (`LoomtideInstall.json`), the manifest
...
function parseArgs(args: string[]): DoctorArgs | ParseHelp {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--project" || arg === "-p") project = path.resolve(args[(i += 1)] ?? "");
      ...
      console.error(`[loomtide doctor] unknown argument "${arg}".`);
...
function checkLocalInstall(checks: DoctorCheck[]): { bundledVersion?: string; bundledSha?: string } {
  ...
  checks.push({
    ...
    label: "CLI build",
    status: "info",
    detail: `loomtide ${stamp.version} (${stamp.commit}, ${stamp.stampStatus})`,
  });
  ...
  checks.push({
    ...
    remediation: nodeMajor >= 18 ? undefined : "Install Node 18+ (Loomtide targets an active LTS).",
  });
  ...
  if (!tgz) {
    checks.push({
      ...
      remediation: "In the dev repo, run scripts/loomtide-pack-bridge.sh (CI should bundle it into @loomtide/cli).",
    });
    return {};
  }
...
function checkProjectWiring(
  ...
  const installCmd = `loomtide install-bridge --project ${project}`;
  ...
  if (!meta || !meta.installMode || !meta.bridgeVersion) {
    checks.push({
      ...
      status: "fail",
      detail: `no bridge in ${METADATA_RELPATH} — bridge not installed by Loomtide`,
  ...
  // The agent-routing front door (LOOMTIDE.md) — present + our marker + current = healthy.
  ...
  if (bundled.bundledVersion && bundled.bundledVersion !== meta.bridgeVersion) {
    checks.push({
      ...
      remediation: `Update available — run: loomtide update --project ${project}  (until update lands: ${installCmd})`,
    });
  }
}
...
function checkRoutingDoc(project: string, checks: DoctorCheck[], installCmd: string): void {
  ...
  if (!existsSync(docPath)) {
    checks.push({
      id: "routing.doc",
      label: "Agent routing (LOOMTIDE.md)",
      status: "warn",
  ...
  if (onDisk === null) {
    checks.push({
      ...
      label: "Agent routing (LOOMTIDE.md)",
      status: "info",
      detail: `present without the Loomtide marker — left as-authored (Loomtide won't manage it)`,
    });
    return;
  }
...
function checkAgentSurface(project: string, meta: InstallMetadata, checks: DoctorCheck[]): void {
  ...
  if (!surface || typeof surface !== "object") {
    push("info", `(optional) agent surface not installed — loomtide install-agent --project ${project}`);
    return;
  }
  ...
  if (surface.state !== "enabled" || !Array.isArray(surface.files)) {
    push(
      ...
      `loomtide install-agent --project ${project}`,
    );
    return;
  }
  ...
  push(
    ...
    current ? undefined : `loomtide install-agent --project ${project}`,
  );
}
...
function printUsage(): void {
  console.log(
    [
      "Usage: loomtide doctor [--project <unity-project-dir>] [--live] [--ci]",
      "",
      "Health check for the local Loomtide install and a project's bridge wiring.",
