/**
 * `loomtide trace` — Replay Verification product surface.
 *
 *   loomtide trace replay --id <id> [--root <dir>] [--trace <path>] [--no-html]
 *   loomtide trace report --id <id> [--root <dir>]
 *
 * `replay` reads `.loomtide/replays/traces/<id>.trace.json`, drives it against the
 * running editor, and writes `.loomtide/replays/reports/<id>.report.{json,html}`
 * plus the captured PNGs under `.loomtide/replays/reports/<id>/actual/`.
 * `report` re-renders the self-contained HTML from an existing report JSON.
 *
 * The verb is the `.loomtide/` integration layer over the replay engine; all
...
interface TraceArgs {
  ...
  id: string;
  root: string;
  /** Override the trace input path (default `.loomtide/replays/traces/<id>.trace.json`). */
  tracePath?: string;
  /**
   * Use the FLAT workspace layout — replay artifacts directly under `--root`
   * (`<root>/traces|reports|baseline`), no nested `.loomtide/`. Default false
   * (standard `<root>/.loomtide/replays/`). The mini-game workspace passes this.
   */
  flat: boolean;
...
export async function run(args: string[]): Promise<number> {
  ...
  try {
    ...
    console.error(`[loomtide trace] fatal: ${message(error)}`);
    return 1;
  }
}
 ...
 * and writes `.loomtide/replays/traces/<id>.trace.json` (green by construction).
 */
async function runRecord(args: TraceArgs): Promise<number> {
  ...
  const baseStop = timed
    ? async () => {
        console.error(`[loomtide trace] recording for ${args.durationSec}s — play your flow now…`);
        await sleep(args.durationSec! * 1000);
      }
    : () =>
        readEnter(
          "[loomtide trace] ▶ Switch to Unity, play your flow, then press Enter here to stop… ",
        );
  ...
  console.error(
    `[loomtide trace] recording "${args.id}" — resetting ${args.scene ?? "the current scene"} to a clean Play-Mode start…`,
  );
  ...
  if (droppedNoTarget > 0) {
    ...
    console.error(
      `[loomtide trace] ignored ${droppedNoTarget} inert tap(s) (no interactive target — backdrop/empty space).`,
    );
  }
  console.error(
    `[loomtide trace] recorded "${args.id}": ${steps} step(s), ${outcomeCount} outcome(s) → ${path.relative(args.root, traceFile)}`,
  );
  ...
  if (args.flat) {
    ...
    console.error(`[loomtide trace] replay it: loomtide trace replay --id ${args.id}`);
  }
  return 0;
}
...
async function replayOneTrace(
  ...
  if (trace.id !== id) {
    console.error(
      `[loomtide trace] warning: trace id "${trace.id}" != "${id}"; using "${id}" for output paths.`,
    );
  }
...
/** Replay every trace under `.loomtide/replays/traces/` and write a roll-up report. */
async function runReplayAll(args: TraceArgs): Promise<number> {
  const paths = layoutFor(args);
  if (args.id) {
    console.error(`[loomtide trace] note: replay-all ignores --id (it runs the whole fleet).`);
  }
  const ids = await discoverTraces(paths.replayTraces);
  if (ids.length === 0) {
    console.error(
      `[loomtide trace] no traces in ${path.relative(args.root, paths.replayTraces)}/ — record or author one first.`,
    );
    return 1;
  }

  console.error(`[loomtide trace] replaying ${ids.length} trace(s)…`);
  ...
  for (const id of ids) {
    try {
      ...
      console.error(`[loomtide trace]   ${id}: ${artifact.status.toUpperCase()}${drift}`);
    } catch (error) {
      ...
      console.error(`[loomtide trace]   ${id}: ERROR — ${detail}`);
    }
  }
  ...
  console.error(
    `[loomtide trace] fleet: ${fleet.status.toUpperCase()} — ${c.pass}/${c.total} pass, ${c.fail} fail, ${c.blocked} blocked, ${c.drift} drift`,
  );
  console.error(`[loomtide trace] fleet report → ${path.relative(args.root, fleetReport)}`);
...
async function runApprove(args: TraceArgs): Promise<number> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      console.error(`[loomtide trace] no report at ${rel} — run 'trace replay --id ${args.id}' first.`);
      return 1;
    }
    throw error;
  }
  ...
  if (!isReplayRunArtifact(parsed)) {
    console.error(`[loomtide trace] ${rel} is not a valid replay report.`);
    return 1;
  }
  ...
  for (const segment of parsed.segments) {
    for (const capture of segment.captures) {
      ...
      if (!isSafePathSegment(capture.id) || !isReplayArtifact(capture.artifact, paths)) {
        console.error(`[loomtide trace] skipping unsafe capture "${capture.id}".`);
        continue;
      }
      ...
      try {
        ...
        console.error(`[loomtide trace] could not approve capture "${capture.id}" (missing actual).`);
      }
    }
  }
  console.error(
    `[loomtide trace] approved ${approved} baseline(s) → ${path.relative(args.root, baselineDir)}`,
  );
  ...
  for (const segment of artifact.segments) {
    for (const capture of segment.captures) {
      ...
      try {
        ...
        anyDrift = true;
        console.error(`[loomtide trace] visual diff failed for "${capture.id}": ${message(error)}`);
      }
    }
  }
...
async function runReport(args: TraceArgs): Promise<number> {
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      console.error(
        `[loomtide trace] no report at ${rel} — run 'trace replay --id ${args.id}' first.`,
      );
      return 1;
    }
    throw error;
  ...
  try {
    ...
    console.error(`[loomtide trace] ${rel} is not valid JSON.`);
    return 1;
  }
  ...
  console.error(`[loomtide trace] html → ${path.relative(args.root, htmlPath)}`);
  return 0;
}
 ...
 * `.loomtide/replays/{reports,baselines}`; flat: `<ws>/{reports,baseline}`), so the
...
function printSummary(
  ...
  console.error(`[loomtide trace] ${id}: ${artifact.status.toUpperCase()}${blocked}`);
  if (artifact.firstDivergence) {
    const d = artifact.firstDivergence;
    console.error(
      `[loomtide trace] first divergence: ${d.kind} @ segment=${d.segment} step=${d.step ?? "-"}`,
    );
    console.error(`[loomtide trace]   expected: ${d.expected}`);
    console.error(`[loomtide trace]   actual:   ${d.actual}`);
  }
  ...
  if (drifted.length) {
    console.error(
      `[loomtide trace] visual drift: ${drifted
        .map((c) => `${c.id} (${((c.diffFraction ?? 0) * 100).toFixed(2)}%)`)
        .join(", ")}`,
    );
  }
  console.error(`[loomtide trace] report → ${path.relative(root, reportJson)}`);
  if (htmlPath) console.error(`[loomtide trace] html   → ${path.relative(root, htmlPath)}`);
}
...
function parseArgs(args: string[]): TraceArgs | { help: true; usageError?: boolean } {
  ...
  if (
    ...
    console.error(`[loomtide trace] unknown subcommand "${sub}".`);
  ...
  const value = (i: number, flag: string): string | undefined => {
    ...
    if (v === undefined || v.startsWith("--")) {
      console.error(`[loomtide trace] ${flag} requires a value.`);
      return undefined;
    }
    return v;
  };
  ...
  for (let i = 1; i < args.length; i += 1) {
    ...
    if (arg === "--id") {
      ...
      if (parsed === null) {
        console.error(
          `[loomtide trace] --state-signal must be <path>:<Component>:<property> (got "${v}").`,
        );
        return { help: true, usageError: true };
      }
      stateSignal = parsed;
      ...
      if (!Number.isFinite(n) || n <= 0) {
        console.error(`[loomtide trace] --duration must be a positive number of seconds (got "${v}").`);
        return { help: true, usageError: true };
      }
      durationSec = n;
    } else if (arg === "--help" || arg === "-h") return { help: true };
    else {
      console.error(`[loomtide trace] unknown argument "${arg}".`);
      return { help: true, usageError: true };
    }
  }
  ...
  if (sub !== "replay-all" && !id) {
    console.error("[loomtide trace] --id <id> is required.");
    return { help: true, usageError: true };
  }
  // `--id` becomes directory and file names (and the trace READ path) — never
  // let it traverse outside `.loomtide/replays/`.
  if (id !== undefined && !isSafePathSegment(id)) {
    console.error(
      `[loomtide trace] --id must not contain path separators or '..' (got "${id}").`,
    );
    return { help: true, usageError: true };
  }
  ...
  if (sub === "record") {
    if (!observe) {
      console.error("[loomtide trace] record requires --observe (the only recording mode in v1).");
      return { help: true, usageError: true };
    }
    ...
    if (scene !== undefined && !isScenePath(scene)) {
      console.error(`[loomtide trace] --scene '${scene}' must be an Assets/**.unity path.`);
      return { help: true, usageError: true };
    }
  }
  return {
...
function printUsage(): void {
  console.error(
    [
      "Usage: loomtide trace <record|replay|replay-all|approve|report> [--id <id>] [options]",
      "",
      ...
      "              --duration <sec>), and write .loomtide/replays/traces/<id>.trace.json.",
      "  replay      Drive .loomtide/replays/traces/<id>.trace.json against the running",
      "              editor and write .loomtide/replays/reports/<id>.report.{json,html},",
      "              diffing each capture against its approved baseline.",
      "  replay-all  Replay EVERY trace under .loomtide/replays/traces/ and write a",
      "              roll-up .loomtide/replays/fleet.report.{json,html}. Exit by worst status.",
      "  approve     Promote the latest run's captures to the approved baseline",
      "              (.loomtide/replays/baselines/<id>/).",
      "  report      Re-render the HTML report from an existing <id>.report.json.",
      "",
      "Options:",
      "  --id <id>         Trace id (required except for replay-all).",
      "  --root <dir>      Project root containing .loomtide/ (default: cwd).",
      "  --trace <path>    Override the trace input path (replay only).",
      "  --flat            Lay replay artifacts directly under --root (traces/, reports/,",
      "                    baseline/) with no nested .loomtide/ — the mini-game workspace layout.",
