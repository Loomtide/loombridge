/**
 * `loomtide plan` — establish/refresh the design target + contract in `.loomtide/`.
 *
 * Walking-skeleton scope (plan §8 M1): the DETERMINISTIC part of `plan` only —
 * scaffold `.loomtide/`, seed a validated `ACCEPTANCE.json` from the genre
 ...
 * layer (`commands/loomtide/plan.md`, `scripts/loomtide-plan`), NOT here. This
...
import { defaultGenreId, knownGenreIds, resolveGenrePack } from "./genre-registry.js";
import { promoteGenreContract, type GenrePromotionResult } from "./genre-contract/promote.js";
import { resolveBriefBundle } from "./brief-bundle.js";
import { formatDesignHardeningAdvisory } from "./genre-contract/design-hardening.js";
import type { GenreContract } from "./genre-contract/types.js";
 ...
 * `commands/loomtide/plan.md`). Returns the detected engine, or `null` + a
...
export interface PlanArgs {
  /** Project root (the dir that will contain `.loomtide/`). */
  root: string;
  genre: string;
  /**
   * Whether `--genre` was passed explicitly (vs defaulted). When false, a re-plan PRESERVES the
   * genre already in STATE instead of resetting it to the registry default — so a bare `loomtide
   ...
   * `/loomtide:plan` slash command) do not have to remember a flag for the
  ...
  note?: string;
  /** Optional operator sign-off image/frame copied into .loomtide/reports/slices/<id>/. */
  signoffPath?: string;
...
export function renderGameSpec(contract: ContractView, genre: string, engine: string): string {
  ...
  const lines: string[] = [
    ...
    "## One-line pitch",
    "",
    `A ${genre} game ("${game}") built with Loomtide on ${engine}` +
...
const DESIGN_README_STUB = `# Design Target (\`.loomtide/design/\`)
...
async function copySignoffArtifact(args: {
  ...
  const ext = /^[.][A-Za-z0-9]{1,12}$/.test(path.extname(args.signoffPath))
    ...
    : ".bin";
  const dest = path.join(args.root, ".loomtide", "reports", "slices", args.sliceId, `signoff${ext}`);
...
async function assetManifestReadiness(paths: LoomtidePaths): Promise<AssetManifestReadiness> {
  try {
...
function renderAssetModeHelp(): void {
  console.error("[loomtide plan] asset strategy required before slice planning:");
  console.error("[loomtide plan]   registry  — use an approved registry pack; slices use those recorded assets.");
  console.error("[loomtide plan]   generated — generate/export assets from the approved hero-shot annotations.");
  console.error("[loomtide plan]   hybrid    — use an approved registry base and generated/manual missing assets.");
}
...
function emitDesignHardeningAdvisory(contract: GenreContract): void {
  for (const line of formatDesignHardeningAdvisory(contract)) {
    console.error(`[loomtide plan] ${line}`);
  }
}
...
export async function runPlan(args: PlanArgs): Promise<number> {
  ...
  if (args.briefPath) {
    if (genreContractPath) {
      console.error("[loomtide plan] pass either --brief or --genre-contract, not both.");
      return 2;
    }
    const resolved = resolveBriefBundle(args.briefPath);
    if ("error" in resolved) {
      console.error(`[loomtide plan] invalid --brief: ${resolved.error}`);
      return 2;
    }
    genreContractPath = resolved.genreContractPath;
    console.error(
      `[loomtide plan] brief bundle → ${path.relative(args.root, genreContractPath)} ` +
  ...
  if (genreContractPath) {
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      console.error(`[loomtide plan] invalid --genre-contract: ${message}`);
      return 2;
    }
  }
  ...
  if (!genrePack) {
    console.error(
      `[loomtide plan] unknown genre "${genre}". Known: ${knownGenreIds().join(", ")}`,
    );
    return 2;
  }
  ...
  if (promoted && !args.force) {
    ...
    if (existing.length > 0) {
      console.error(
        `[loomtide plan] NOT ready — --genre-contract would mix promoted artifacts with existing ${existing.join(", ")}. Re-run with --force to replace them.`,
      );
      return 2;
    }
  }
  ...
  if (args.force || !(await fileExists(paths.acceptance))) {
    ...
    const feelSpec = {
      genre,
      source: "derived from ACCEPTANCE.json `feel` by `loomtide plan`",
      metrics: feel,
    };
...
`, "utf-8");
  ...
  console.error(`[loomtide plan] genre=${genre} engine=${args.engine} root=${args.root}`);
  if (created.length) console.error(`[loomtide plan] created: ${created.join(", ")}`);
  if (kept.length) console.error(`[loomtide plan] kept (unchanged): ${kept.join(", ")}`);
  ...
  console.error(`[loomtide plan] design target: ${design.status}${stale ? " (CHANGED since approval — re-approve)" : ""}`);

  let assetManifest = await assetManifestReadiness(paths);
  if (args.assetMode !== undefined) {
    if (!gateOk || !design.pngSha256) {
      console.error(
        "[loomtide plan] NOT ready — choose an asset strategy after the Design Target is approved + frozen. " +
          "Run `loomtide design set/approve`, then `loomtide plan --asset-mode <registry|generated|hybrid>`.",
      );
      return 1;
    }
    ...
    try {
      ...
      console.error(`[loomtide plan] NOT ready — ${error instanceof Error ? error.message : String(error)}`);
      return 2;
    }
    console.error(
      outcome === "created"
        ? `[loomtide plan] recorded asset strategy draft: ${args.assetMode} → ${rel(paths.assetManifest)}`
        : `[loomtide plan] kept existing asset manifest: ${rel(paths.assetManifest)} (use --force to replace the draft)`,
    );
  ...
  console.error(`[loomtide plan] asset manifest: ${assetLabel}`);
  if (assetManifest.status === "invalid" && assetManifest.reason) {
    console.error(`[loomtide plan] asset manifest validation failed: ${assetManifest.reason}`);
  }
  ...
  if (mode === "design") {
    ...
    // flag, not the gate — so the common path (and the `/loomtide:plan` slash
    ...
    if (!gateOk && !args.allowMissingDesignTarget) {
      ...
      console.error(
        `[loomtide plan] NOT ready — ${reason}. ` +
          "Establish/re-approve via `loomtide design set/approve` (see commands/loomtide/plan.md §3c), then re-run. " +
    ...
    if (!gateOk && args.allowMissingDesignTarget) {
      console.error(
        "[loomtide plan] WARNING: completing without an approved Design Target (--allow-missing-design-target). " +
          "Polish will be ungrounded; `build` will still block until the target is approved + frozen.",
      );
    }
    if (!gateOk) {
      console.error(
        "[loomtide plan] next: establish the Design Target (annotated hero shot) in .loomtide/design/ via `loomtide design`, then re-run `loomtide plan`.",
      );
      return 0;
    }
    ...
    if (!assetManifest.approved) {
      ...
      if (assetManifest.status === "missing") {
        console.error(
          "[loomtide plan] NOT ready — no approved ASSET_MANIFEST.json. " +
            "Record the user's choice with `loomtide plan --asset-mode hybrid` (or registry/generated), " +
            "fill the required roles/assets, approve the manifest, then re-run.",
        );
      } else if (assetManifest.status === "draft") {
        console.error(
          `[loomtide plan] NOT ready — ASSET_MANIFEST.json is draft (${assetManifest.mode}). ` +
            "Fill required assets/slice bindings and set status approved before scaffolding slices.",
        );
      } else {
        console.error(
          "[loomtide plan] NOT ready — ASSET_MANIFEST.json is invalid. Fix validation errors before scaffolding slices.",
        );
      }
      return 1;
    }
    ...
    plan = instantiated;
    console.error(
      `[loomtide plan] scaffolded roadmap: ${instantiated.slices.length} slices → ${rel(paths.slices)}`,
    );
    renderPlanStatusEcho(await computeStatusModel({ paths, plan, state, design }));
    const next = nextUnblockedSlice(plan);
    if (next) {
      console.error(
        `[loomtide plan] Plan next slice: ${next.id} — ${next.title}`,
      );
      console.error(`[loomtide plan]   skill: ${next.skill}`);
      console.error(`[loomtide plan]   feel: ${next.feelIntent}`);
      console.error(`[loomtide plan]   gates: ${next.acceptance.gates.join(", ")}`);
      console.error(
        "[loomtide plan] next: review the slice above, then run `loomtide build` to build it.",
      );
    } else {
      console.error("[loomtide plan] next: all slices approved.");
    }
    return 0;
  }
  ...
  if (mode === "await-approval") {
    ...
    if (!args.go) {
      console.error(`[loomtide plan] Approving ${names} (verified ✓) and advancing — ok?`);
      console.error(
        "[loomtide plan] Approval is the plan↔build human seam. In an agent session, approve this prompt; the agent will run the internal approval action. No change made now.",
      );
      console.error(`[loomtide plan] Next: say "approve ${awaiting[0]?.id ?? names}" or run /loomtide:plan to approve and advance.`);
      return 0;
    }
    ...
    for (const slice of awaiting) {
      if (slice.state === "built") {
        refused = true;
        console.error(`[loomtide plan] ${slice.id}: NOT approved — verify it first (\`loomtide verify --slice ${slice.id}\`).`);
        continue;
      }
      if (!slice.proof?.checkpointId) {
        refused = true;
        console.error(`[loomtide plan] ${slice.id}: NOT approved — proof.checkpointId is missing; re-run \`loomtide verify --slice ${slice.id}\`.`);
        continue;
      }
      const done = await isSliceDone(slice, paths);
      if (!done.ok) {
        refused = true;
        console.error(`[loomtide plan] ${slice.id}: NOT approved — doneness refused:`);
      ...
      if (args.signoffPath) {
        try {
          ...
          refused = true;
          const message = error instanceof Error ? error.message : String(error);
          console.error(`[loomtide plan] ${slice.id}: NOT approved — ${message}`);
          continue;
        }
      }
    ...
    if (approvals.size > 0) {
      ...
      plan = nextPlan;
      console.error(`[loomtide plan] approved: ${[...approvals.keys()].join(", ")}`);
      const next = nextUnblockedSlice(plan);
      if (next) {
        renderPlanStatusEcho(await computeStatusModel({ paths, plan, state, design }));
        console.error(`[loomtide plan] next: ${next.id} — ${next.title}`);
        console.error("[loomtide plan] Next: run /loomtide:build or say continue.");
      } else {
        state = await updateState(paths, { currentBuild: null });
        renderPlanStatusEcho(await computeStatusModel({ paths, plan, state, design }));
        console.error(`[loomtide plan] next: all approved.`);
        console.error("[loomtide plan] Next: all slices are approved; ask the agent to certify done.");
      }
    }
    if (refused) return 1;
    return 0;
  ...
  // `loomtide build`. NO mutation in S1b — `--go` has no extra effect here yet.
  if (mode === "plan-slice") {
    if (!assetManifest.approved) {
      renderAssetModeHelp();
      console.error(
        "[loomtide plan] NOT ready — slice planning requires an approved ASSET_MANIFEST.json. " +
          "Slices must consume manifest-bound asset ids, not search the registry silently.",
      );
      return 1;
    }
    const next = nextUnblockedSlice(plan!)!;
    console.error(`[loomtide plan] Plan next slice: ${next.id} — ${next.title}`);
    console.error(`[loomtide plan]   skill: ${next.skill}`);
    console.error(`[loomtide plan]   feel: ${next.feelIntent}`);
    console.error(`[loomtide plan]   gates: ${next.acceptance.gates.join(", ")}`);
    console.error(`[loomtide plan] next: review the slice above, then run \`loomtide build\` to build it.`);
    if (args.go) {
      console.error(
        "[loomtide plan] (--go has no effect on a slice plan yet — `plan` makes no slice mutation in S1b; " +
  ...
  console.error(
    `[loomtide plan] All ${plan!.slices.length} slices approved — this game's roadmap is complete.`,
  );
  console.error(
    "[loomtide plan] next: add a new slice to the roadmap (or a new genre pack) to extend the game. " +
      "Run `loomtide doneness` to certify the composed build.",
  );
  return 0;
}
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--genre") { genre = args[(i += 1)] ?? genre; genreExplicit = true; }
      ...
      if (!value || !ASSET_MANIFEST_MODES.has(value as AssetManifestMode)) {
        console.error("[loomtide plan] --asset-mode must be registry, generated, or hybrid.");
        return { help: true };
      }
      ...
      console.error(`[loomtide plan] unknown argument "${arg}".`);
      return { help: true };
    }
  }
...
function resolveEngine(parsed: ParsedArgs): { engine: string } | { exitCode: number } {
  if (parsed.engine !== undefined) {
    if (parsed.engine !== "unity") {
      console.error(
        `[loomtide plan] --engine ${parsed.engine} is not supported; Loomtide currently supports Unity only`,
      );
      return { exitCode: 2 };
    }
  ...
  if (detected.engine === null) {
    console.error(`[loomtide plan] ${detected.reason}`);
    return { exitCode: 2 };
  }
  if (detected.engine !== "unity") {
    console.error(
      `[loomtide plan] detected ${detected.engine}, but Loomtide currently supports Unity only`,
    );
    return { exitCode: 2 };
  }
  console.error(`[loomtide plan] detected engine: ${detected.engine}`);
...
function printUsage(): void {
  console.log(
    [
      "Usage: loomtide plan [options]",
      "",
      "State-driven: dispatches by `.loomtide/` state — establish the Design",
      ...
      "                  .loomtide/ACCEPTANCE.json, SLICES.json, and",
      ...
      "                  .loomtide/ASSET_MANIFEST.json. Roadmap scaffolding still",
      ...
      "                  .loomtide/reports/slices/<id>/ and hashed at approval",
