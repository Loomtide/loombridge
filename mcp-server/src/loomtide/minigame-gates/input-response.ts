...
function gradeState(
  ...
  if (!evidence) {
    return {
      ...
      nextAction: `Run \`loomtide minigame capture\` (it drives the declared '${ir.tap}' tap and snapshots around it), then re-run. This is a capture gap, NOT a game result.`,
    };
  }
