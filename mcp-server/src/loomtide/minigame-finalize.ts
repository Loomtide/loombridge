/**
 * `loomtide minigame finalize` — turn a placeholder `setup`/`init` contract into a
...
/** Build a canonical Loomtide locator (`SceneName:/Path`, or bare `/Path`). */
...
export async function runFinalize(argv: string[], root: string = process.cwd()): Promise<number> {
  ...
  if ("error" in parsed) {
    console.error(`[loomtide minigame] finalize: ${parsed.error}.`);
    printUsage();
    return 2;
  }
  ...
  try {
    ...
    console.error(
      `[loomtide minigame] finalize: could not load contract ${parsed.contractPath}: ${message(error)}`,
    );
    return 2;
  }
  ...
  if (parsed.outcomeGated && parsed.outcomeGated.length > 0) {
    ...
    if (unknown.length > 0) {
      console.error(
        `[loomtide minigame] finalize: --outcome-gated names unknown state(s): ${unknown.join(", ")} ` +
  ...
  if (unsupported.length > 0) {
    console.error(
      `[loomtide minigame] finalize: this draft declares ${unsupported.join(", ")}, which finalize does not ` +
  ...
  if (load.kind === "empty") {
    console.error(
      `[loomtide minigame] finalize: capture pack missing at ${rel(root, parsed.capturesDir)} — ` +
  ...
  if (load.kind === "missing") {
    console.error(
      `[loomtide minigame] finalize: missing capture(s) for state(s): ${load.states.join(", ")} — ` +
  ...
  try {
    ...
    console.error(`[loomtide minigame] finalize: ${message(error)}`);
    return 1;
  }

  if (!result.contract) {
    console.error("[loomtide minigame] finalize: could not infer every required role from the captures:");
    ...
    console.error(
      "[loomtide minigame] finalize: refusing to write placeholder/invented locators — capture the missing screen(s) " +
    ...
    if (gateableStates.length > 0) {
      console.error(
        `[loomtide minigame] finalize: if a screen is gated by gameplay OUTCOME (a win behind correct answers / RNG) ` +
  ...
  if (parsed.inputResponse) {
    ...
    if ("error" in applied) {
      console.error(`[loomtide minigame] finalize: ${applied.error}`);
      return 2;
    }
  ...
  if (parsed.safeAreaInsets) {
    ...
    try {
      ...
      console.error(`[loomtide minigame] finalize: --safe-area-insets produced an invalid contract: ${message(error)}`);
      return 2;
    }
  }
  ...
  try {
    ...
    console.error(`[loomtide minigame] finalize: could not write ${outPath}: ${message(error)}`);
    return 1;
  }
...
function printUsage(): void {
  console.error(
    [
      "Usage: loomtide minigame finalize --contract <path> --captures <dir> [--trace-root <dir>] [--output <path>]",
      "",
