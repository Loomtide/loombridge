...
rename** to make a run pass full `loomtide tuning-report` validation against
`telemetry.json`.
...
```
loomtide tuning-report --genre 3d-topdown-arena --runs <dir-of-*_summary.json+*_events.jsonl>
```
