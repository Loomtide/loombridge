...
Loomtide can measure for a 3D shooter today versus what is deferred, so no row is ever silently
treated as green. It pairs with:

- `acceptance.json` / `slices.json` — the plan-time templates (`plan --genre 3d-shooter`).
- `mcp-server/src/loomtide/genre-contract/genre-packs/3d-shooter/hint-card.json` — the elicitation
