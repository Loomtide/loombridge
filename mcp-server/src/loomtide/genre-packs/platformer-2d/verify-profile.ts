/**
 * `loomtide verify --profile <id>` — the verify-first entry (S5b).
 *
 ...
 * The public `loomtide verify --profile` command resolves that path under the external verification workspace
 * (`~/.loomtide/projects/<id>/feel/reports/feel-profile.json`) so a verify-first
...
import { detectEngine } from "../../plan.js";
...
import { resolveBuildStamp, formatBuildStamp, type BuildStamp } from "../../build-stamp.js";
...
export interface ProfileVerifyReport {
  ...
  schemaVersion: "1";
  producedAt: string;
  /** Which loomtide build graded this report (F5 #14: audit provenance). */
...
function nextActionFor(
  ...
  if (!hasMeasurements) {
    if (!engineOk) {
      return "Run this from your Unity project root (the folder with Assets/ and ProjectSettings/) so Loomtide can detect the project.";
    }
    return `No measurements yet — have Loomtide measure your player (run/jump/coyote/buffer) and re-run to grade against the '${report.profile.id}' profile.`;
  }
...
export interface VerifyProfileArgs {
  /** Project root (the dir that contains the Unity project / `.loomtide/`). */
  root: string;
...
/** Render the report to stderr in the loomtide developer voice. */
function renderReport(report: ProfileVerifyReport): void {
  const p = report.profile;
  console.error(`[loomtide verify] profile=${p.id} (${p.title}) — ${p.summary}`);
  if (p.exemplars && p.exemplars.length > 0) {
    console.error(`[loomtide verify] feels like: ${p.exemplars.join(", ")}`);
  }
  if (report.engine.engine !== "unity") {
    ...
    console.error(
      `[loomtide verify] engine: ${detected} — ${report.engine.reason ?? "verify-first targets Unity 2D platformers"}`,
    );
  }
  // Headline first — the single most important line.
  console.error(`[loomtide verify] ${report.headline}`);
  console.error(`[loomtide verify] produced by ${formatBuildStamp(report.producedBy)}`);
  ...
  if (report.alsoMeasured.length > 0) {
    console.error(`[loomtide verify] also measured (unbanded — informational, not graded):`);
  ...
  if (report.captureCoverage.length > 0) {
    console.error(`[loomtide verify] capture coverage:`);
  ...
  if (report.mechanisms.status !== "not_applicable") {
    console.error(
      `[loomtide verify] mechanisms (${report.mechanisms.status}) — required present / forbidden absent, verified behaviorally:`,
    );
  ...
  if (reach.status === "not_run") {
    console.error(
      `[loomtide verify] reachability (not_run) — ${reach.reason ?? "no level layout supplied"}`,
    );
  } else {
    ...
    const envStr = env
      ...
      : "";
    console.error(
      `[loomtide verify] reachability (${reach.status})${envStr} — every collectible within the profile's swapped envelope:`,
    );
  ...
  const c = report.confidence;
  console.error(
    `[loomtide verify] status=${report.status} | ${s.pass} pass, ${s.fail} fail, ${s.notMeasured} not measured (of ${s.total})`,
  );
  console.error(
    `[loomtide verify] confidence | ${c.verified} verified, ${c.reported} reported, ${c.rejected} rejected, ${c.unmeasured} unmeasured`,
  );
  console.error(`[loomtide verify] next: ${report.nextAction}`);
}
...
export function unknownProfileMessage(id: string): string {
  return `[loomtide verify] unknown --profile "${id}". Shipped profiles: ${SHIPPED_PROFILE_IDS.join(", ")}.`;
}
