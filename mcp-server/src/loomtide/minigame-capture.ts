/**
 * `loomtide minigame capture` — drive the recorded happy-path trace through the
 * live bridge and emit the verify capture pack, so Step 2 is ONE command, not a
 * hand-written script.
 *
 *   loomtide minigame capture --contract <p> --trace-root <dir> --captures <out> [--id <trace>] [--settle <ms>]
 *
...
import { changedObjectIds } from "./minigame-gates/input-response.js";
...
import type { BackgroundCandidates, MinigameCaptureState } from "./minigame-gates/types.js";
import { BACKGROUND_COVERAGE_GATE, BACKGROUND_SEAM_GATE } from "./minigame-gates/evaluators.js";
import { BACKGROUND_DATA_FILE } from "./minigame-gates/run-minigame-gates.js";
...
import { ICON, unityConnectionHint } from "./cli-ui.js";
      ...
      action.do === "wait-for-condition"
        ? await dispatchConditionWithGestureRecovery(driver, action, lastGesture, {
            onRetry: (attempt, travelPx) =>
              console.error(
                `[loomtide minigame] capture: phase gate '${action.expected}' not met on ${device.id} — re-driving the gesture (attempt ${attempt}, travel ${Math.round(travelPx)}px).`,
              ),
          })
...
async function captureLive(
  ...
  try {
    ...
    try {
      ...
      console.error(`[loomtide minigame] capture: background discovery skipped (${message(error)}).`);
...
export async function runCapture(argv: string[], root: string = process.cwd()): Promise<number> {
  ...
  if ("error" in parsed) {
    console.error(`[loomtide minigame] capture: ${parsed.error}.`);
    printUsage();
    return 2;
  }
  ...
  try {
    ...
    console.error(`[loomtide minigame] capture: could not load contract ${parsed.contractPath}: ${message(error)}`);
    return 2;
  }
  ...
  try {
    ...
    console.error(
      `[loomtide minigame] capture: could not load trace '${traceId}' under ${rel(root, parsed.traceRoot)}: ${message(error)} ` +
        "(record one first with `loomtide trace record --observe`).",
    );
    return 2;
  }
  ...
  if (isDraftContract(contract)) {
    ...
    if (reordered !== contract) {
      ...
      try {
        ...
        console.error(
          `[loomtide minigame] capture: reordered states to the demonstrated play order → ${contract.interactionFlow.happyPath.join(" → ")}.`,
        );
      } catch {
  ...
  try {
    ...
    console.error(hint ? hint.join("\n") : `[loomtide minigame] capture: drive failed: ${message(error)}`);
    return 1;
  }

  if ("blocked" in result) {
    console.error(
      `[loomtide minigame] capture: could not drive the game — ${result.blocked}. ` +
...
export function buildCaptureSummary(
  ...
  if (traceGap.length > 0) {
    ...
    if (gateableGap.length > 0) {
      lines.push(
        ...
        `     loomtide minigame finalize --contract ${rel(root, contractPath)} --captures ${rel(root, capturesDir)} --outcome-gated ${gateableGap.map((u) => u.stateId).join(",")}`,
      );
    }
  }
  ...
  if (response && response.changed.length > 0) {
    ...
    lines.push(
      ...
      `      loomtide minigame finalize --contract ${rel(root, contractPath)} --captures ${rel(root, capturesDir)} --input-response ${response.stateId}:${tapId}:${shown.join(",")}`,
    );
  } else if (response) {
    lines.push(
      "",
  ...
  if (!backgroundAlreadyBound && candidates) {
    if (candidates.camera && candidates.recommended.length > 0) {
      ...
      lines.push(
        ...
        `      loomtide minigame finalize --contract ${rel(root, contractPath)} --captures ${rel(root, capturesDir)} --background auto`,
      );
...
function printUsage(): void {
  console.error(
    [
      "Usage: loomtide minigame capture --contract <path> --trace-root <dir> --captures <out> [--id <trace>] [--settle <ms>]",
      "",
