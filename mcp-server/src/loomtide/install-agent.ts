/**
 * `loomtide install-agent` — OPT IN to Loomtide's agent commands + skills, installed
 ...
 * job (scripts/loomtide-install-locally.sh). Skipping is the DEFAULT (do nothing); opting
 * out is `--remove` and is REMEMBERED in the committed install record (no nagging).
 *
 * The preference is a tri-state `agentSurface` block in ProjectSettings/LoomtideInstall.json
 * (the already-committed record — NOT a .loomtide/ file, which consumers gitignore):
 ...
 * managed-marker (like LOOMTIDE.md): editing a file — or deleting its marker — changes its
 * bytes so its sha256 no longer matches the ledger, and Loomtide then treats it as the
 * user's (skipped on refresh, left in place on --remove).
 *
 * Layout installed into the project:
 *   .claude/commands/loomtide/<name>.md         (Claude slash commands)
...
const MARKER_RE = /<!--\s*loomtide:agent-surface\s+v(\d+)\b/;
...
function parseArgs(args: string[]): InstallAgentArgs | ParseHelp {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--project" || arg === "-p") project = args[(i += 1)] ?? "";
      ...
      console.error(`[loomtide install-agent] unknown argument "${arg}".`);
      return { help: true, usageError: true };
    }
  }
  if (!project) {
    console.error("[loomtide install-agent] --project <unity-project-dir> is required.");
    return { help: true, usageError: true };
  }
...
function resolvePayloadDir(): string {
  ...
  throw new RuntimeFailure(
    "no agent-surface payload bundled with this CLI. In the dev repo, run scripts/build-agent-surface.mjs " +
      "(CI's prepack builds it into @loomtide/cli).",
  );
}
...
function markerComment(): string {
  return (
    `<!-- loomtide:agent-surface v${AGENT_SURFACE_MARKER_VERSION} — GENERATED by \`loomtide install-agent\`. ` +
    "Do NOT hand-edit; re-run install-agent to refresh. Editing this file (or deleting this marker) makes " +
    "Loomtide treat it as yours: skipped on refresh, left in place on --remove. -->"
  );
}
...
const GITATTRIBUTES_BYTES =
  "# Loomtide-managed — keep LF so the install ledger's sha matches on every OS (Windows core.autocrlf included).\n" +
  "* text eol=lf\n";
 ...
 * the LEDGER is Loomtide-managed and gets rewritten to the current payload; a target that
...
function installSurface(project: string, dryRun: boolean): number {
  ...
  console.log(`==> ${prior?.state === "enabled" ? "Refreshing" : "Installing"} the Loomtide agent surface in ${project}`);
  ...
  try {
    ...
    // or renamed command/skill) must not strand. Where it is still Loomtide-managed on disk
  ...
  console.log(`  -> ProjectSettings/LoomtideInstall.json (agentSurface: enabled, ${ledger.length} managed file(s))`);
  console.log("");
  console.log("==> Done. Commit .claude/ (+ .codex/) + ProjectSettings/LoomtideInstall.json so teammates get it via git pull.");
  return 0;
}
...
/**
 * Opt out: delete Loomtide-managed files (ONLY where on-disk sha256 matches the ledger),
...
function removeSurface(project: string, dryRun: boolean): number {
  ...
  console.log(`==> Removing the Loomtide agent surface from ${project}`);
  ...
  if (kept.length) {
    console.log(`  !! ${kept.length} hand-edited file(s) LEFT in place (yours — not Loomtide's to remove):`);
  ...
  const patch = meta ? { agentSurface: record } : { schemaVersion: 1, agentSurface: record };
  writeInstallRecord(project, patch, dryRun);
  console.log("  -> ProjectSettings/LoomtideInstall.json (agentSurface: declined) — install/update stay silent now");
  console.log("");
  console.log("==> Done. Re-enable any time: loomtide install-agent --project " + project);
  return 0;
}
...
export function printAgentSurfaceHintIfUnset(project: string): boolean {
  ...
  console.log(`  -> ${HINT_PREFIX} — loomtide install-agent --project ${project}`);
  return true;
}

/**
 * `loomtide update`: UNSET → hint; "declined" → silent; "enabled" → REFRESH the surface to
...
function printUsage(): void {
  console.log(
    [
      "Usage: loomtide install-agent --project <unity-project-dir> [options]",
      "",
      "OPT IN to Loomtide's agent commands + skills, installed INTO your Unity project's",
      "repo (committed, so teammates get them via git pull). Skipping is the DEFAULT — do",
      "nothing. The choice is remembered in ProjectSettings/LoomtideInstall.json, so one",
      "dev decides and everyone's `loomtide update` behaves identically after a pull.",
      "",
      "Installs (real committed files, never ~/.claude or ~/.codex):",
      "  .claude/commands/loomtide/*.md   Claude slash commands",
      "  .claude/skills/<name>/**         Claude skills",
...
export async function run(args: string[]): Promise<number> {
  ...
  try {
    ...
    if (error instanceof UsageError) {
      console.error(`[loomtide install-agent] ${error.message}`);
      return 2;
    }
    const message = error instanceof Error ? error.message : String(error);
    console.error(`[loomtide install-agent] ${message}`);
    return 1;
  }
}
