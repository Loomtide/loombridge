 ...
 *  - `engine-unsupported-today`  → outside Loomtide's substrate today (e.g. 3D / Fusion-replicated state).
 */
...
export interface GenreContract {
   ...
   * `loomtide plan` propagates it VERBATIM into the generated acceptance contract's
   * `verification.requiredEvidenceClasses` (where `loomtide doneness` enforces it). Present ⇒ validated
