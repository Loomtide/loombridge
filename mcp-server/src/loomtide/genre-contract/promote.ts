import { assertValidSlicePlan, SLICES_SCHEMA_VERSION, type SlicePlan } from "../slices.js";
import {
...
function verificationOverrides(contract: GenreContract): AcceptanceContract["verification"] {
  ...
  // contract's `verification.requiredEvidenceClasses`, where `loomtide doneness` enforces it. Absent ⇒
...
export function promoteGenreContract(input: unknown, options: GenrePromotionOptions = {}): GenrePromotionResult {
  ...
  const report: GenrePromotionReport = {
    ...
    generatedAcceptance: ".loomtide/ACCEPTANCE.json",
    generatedSlices: ".loomtide/SLICES.json",
