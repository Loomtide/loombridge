...
export function buildDesignDocCoverageReport(contract: GenreContract): DesignDocCoverageReport {
  ...
  // (d) playtest acceptance protocol — HONEST LIMIT: no dedicated field; proxied.
  {
    ...
    items.push({
      ...
      rationale:
        proxies === 2
          ? "enforced gate thresholds AND human-oracle checks are both declared — a reasonable acceptance-protocol proxy. (Loomtide has no first-class acceptance-protocol section; graded from proxies.)"
          : proxies === 1
  ...
  // (RCL §6); otherwise falls back to the per-slice gates/gaps PROXY (unchanged partial/absent semantics).
  {
    ...
    if (declared.length > 0) {
      ...
      // generated acceptance contract, where `loomtide doneness` enforces each class is `present`.
      items.push({
 ...
 * prepends its own, e.g. "[loomtide plan]  "). Pure: deterministic transform, no I/O. Lines are the
