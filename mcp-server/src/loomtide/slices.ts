 ...
 *
 * A Loomtide project is built one polished slice at a time: `plan` emits an
 * ordered, dependency-aware DAG (`.loomtide/SLICES.json`) from a genre template,
...
import type { LoomtidePaths } from "./state.js";
...
export interface SliceProof {
  ...
  startedAt?: string;
  /** Per-slice verdict path: .loomtide/reports/slices/<sliceId>.verdict.json. */
  verdictPath?: string;
  /** Paths under .loomtide/verify/<sliceId>/ that MUST exist to certify. */
  captureManifest?: string[];
  /** The .loomtide-fixtures/<sliceId>/ snapshot id; null until verified. */
  ...
  approvalNote?: string;
  /** Durable root-relative sign-off artifact copied under .loomtide/reports/slices/<id>/. */
...
/** The slice DAG written to `.loomtide/SLICES.json`. */
  ...
  if (!Array.isArray(input.slices)) {
    ...
    input.slices.forEach((slice, i) => {
      ...
      if (!isNonEmptyString(slice.id)) {
        ...
        // A slice id is a SINGLE `.loomtide/` path segment (it names the per-slice
        // verdict/verify/fixture dirs). Reject traversal/unsafe ids at rest so a
        // hand-edited SLICES.json can never steer a write outside `.loomtide/`
        // (the path builders also assert, but the contract must be valid at rest).
        issues.push(
          `${p}.id: '${slice.id}' is not a safe slice id (must match ${SAFE_SLICE_ID_RE} — a single .loomtide/ path segment, no traversal).`,
        );
...
/**
 * Deterministic dispatch precedence for `loomtide plan` (plan §'Settled command
...
/**
 * A slice id is a single path segment under `.loomtide/` — it MUST NOT escape
 * (no `..`, no `/`, no absolute path). A hand-edited SLICES.json must not be able
 * to steer the per-slice verdict/verify dir outside `.loomtide/`. Mirrors the
...
export function assertSafeSliceId(sliceId: string): string {
  if (typeof sliceId !== "string" || !SAFE_SLICE_ID_RE.test(sliceId)) {
    throw new Error(
      `unsafe slice id "${sliceId}" — must match ${SAFE_SLICE_ID_RE} (a single .loomtide/ path segment; no traversal).`,
    );
  }
  return sliceId;
}

/** `.loomtide/reports/slices/<sliceId>.verdict.json` — the per-slice verdict path (S2a). */
export function getSliceVerdictPath(paths: LoomtidePaths, sliceId: string): string {
  return path.join(paths.reports, "slices", `${assertSafeSliceId(sliceId)}.verdict.json`);
}

/** `.loomtide/reports/slices/<sliceId>.diagnostic.json` — non-binding re-verify output. */
export function getSliceDiagnosticPath(paths: LoomtidePaths, sliceId: string): string {
  return path.join(paths.reports, "slices", `${assertSafeSliceId(sliceId)}.diagnostic.json`);
}

/** `.loomtide/verify/<sliceId>/` — the per-slice captured-op-output dir (S2a). */
export function getSliceVerifyDir(paths: LoomtidePaths, sliceId: string): string {
  return path.join(paths.verifyInputs, assertSafeSliceId(sliceId));
}

/** `.loomtide-fixtures/<sliceId>/` — the per-slice resumable checkpoint dir. */
export function getSliceFixtureDir(paths: LoomtidePaths, sliceId: string): string {
  return path.join(paths.root, ".loomtide-fixtures", assertSafeSliceId(sliceId));
}

// ── fs helpers (separate from the pure fns) ──────────────────────────────────

/**
 * Read + validate `.loomtide/SLICES.json`; null when absent (the legacy
 * whole-game model). Mirrors the JSON read idiom in state.ts/doneness.ts.
 */
export async function readSlicePlan(paths: LoomtidePaths): Promise<SlicePlan | null> {
  let raw: string;
  try {
...
/** Validate then write `.loomtide/SLICES.json` (pretty-printed, trailing newline). */
export async function writeSlicePlan(paths: LoomtidePaths, plan: SlicePlan): Promise<void> {
