/**
 * `loomtide minigame` — mini-game helper verb (S6e).
 *
 ...
 * bundle):
 *
 *   loomtide minigame baseline approve --contract <c> --captures <d> [--ref <dir>]
 *   loomtide minigame baseline status  --contract <c> [--ref <dir>]
 *
 ...
 * game is never touched, and the `.loomtide/` build tree is never scaffolded.
 */
...
import { runMinigameGates } from "./minigame-gates/run-minigame-gates.js";
...
function parse(argv: string[]): Parsed {
  ...
  if (group === "init") {
    ...
    if ("error" in parsed) {
      console.error(`[loomtide minigame] ${parsed.error}.`);
...
async function runApprove(args: BaselineArgs): Promise<number> {
  if (args.capturesDir === undefined) {
    console.error("[loomtide minigame] baseline approve requires --captures <dir>.");
    return 2;
  }
  ...
  try {
    ...
    console.error(`[loomtide minigame] could not load contract ${args.contractPath}: ${error instanceof Error ? error.message : String(error)}`);
    return 2;
  }

  const refDir = resolveBaselineRef(contract, args.root, args.ref);
  if (!refDir) {
    console.error("[loomtide minigame] no baseline target — set contract.baseline.ref or pass --ref <dir>.");
    return 2;
  }
  ...
  if (status !== "pass") {
    console.error(
      `[loomtide minigame] REFUSED to approve a non-pass run (status=${status}). ` +
        "Only a PASS verification may be frozen as a baseline — fix the failures/captures and re-run `loomtide verify --minigame` to green first.",
    );
    return 1;
  }
  ...
  try {
    ...
    console.error(
      `[loomtide minigame] baseline APPROVED for '${contract.id}' → ${path.relative(args.root, refDir)} ` +
        `(${manifest.states.length} state(s), ${manifest.masks.length} mask(s), capturedAt ${manifest.capturedAt}).`,
    );
    console.error("[loomtide minigame] future `loomtide verify --minigame` runs now enforce this baseline (drift = baseline regression).");
    return 0;
  } catch (error) {
    console.error(`[loomtide minigame] failed to write baseline bundle: ${error instanceof Error ? error.message : String(error)}`);
    return 1;
  }
}
...
async function runStatus(args: BaselineArgs): Promise<number> {
  ...
  if (!manifest) {
    console.error(`[loomtide minigame] no approved baseline at ${path.relative(args.root, refDir)} — run \`loomtide minigame baseline approve\`.`);
    return 0;
  }
  console.error(
    `[loomtide minigame] baseline for '${manifest.contractId}' @ ${path.relative(args.root, refDir)}`,
  );
  console.error(
...
async function runInit(args: InitArgs): Promise<number> {
  ...
  if (!args.output) {
    ...
    console.error(
      `[loomtide minigame] scaffolded contract for '${args.id}' (stdout). ` +
        "Edit scenes / locators / states to match your game, then run `loomtide verify --minigame`.",
    );
    return 0;
  }
  ...
  try {
    ...
    if (!args.force) {
      console.error(
        `[loomtide minigame] ${path.relative(process.cwd(), args.output)} already exists — refusing to overwrite. ` +
  ...
  try {
    ...
    console.error(`[loomtide minigame] failed to write ${args.output}: ${error instanceof Error ? error.message : String(error)}`);
    return 1;
  }
  console.error(
    `[loomtide minigame] wrote a starter contract for '${args.id}' → ${path.relative(process.cwd(), args.output)}.`,
  );
  console.error(
    "[loomtide minigame] next: edit scenes / requiredInFrame locators / states to match your game, capture its screens, then `loomtide verify --minigame`.",
  );
  return 0;
}
...
function printUsage(): void {
  console.log(
    [
      "Usage: loomtide minigame <command> [options]",
      "",
      ...
      "",
      "setup/capture/finalize options: run `loomtide minigame <cmd> --help` for their flags.",
      "scan/sync options: run `loomtide minigame <cmd> --help` for their flags.",
      "",
      "init options:",
...
export async function run(argv: string[]): Promise<number> {
  ...
  try {
    ...
    console.error(`[loomtide minigame] fatal: ${error instanceof Error ? error.message : String(error)}`);
    return 1;
  }
}
