/**
 * `loomtide install-bridge` — install the Loomtide Unity bridge into a consumer
 * Unity project WITHOUT cloning the Loomtide repo.
 *
 ...
 * `Packages/com.loomtide.unitybridge/`, EXCLUDING Tests/ (the legacy RUN-1 #62
...
function resolveBundledTarball(override?: string): string {
  ...
  if (!tgz) {
    throw new RuntimeFailure(
      "no bundled bridge tarball found. In the dev repo, run scripts/loomtide-pack-bridge.sh first.",
    );
  }
  return tgz;
}
...
function parseArgs(args: string[]): InstallArgs | ParseHelp {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--project" || arg === "-p") project = args[(i += 1)] ?? "";
      ...
      console.error(`[loomtide install-bridge] unknown argument "${arg}".`);
      return { help: true, usageError: true };
    }
  }
  if (!project) {
    console.error("[loomtide install-bridge] --project <unity-project-dir> is required.");
    return { help: true, usageError: true };
  }
...
/**
 * Install / refresh the agent-routing front door (LOOMTIDE.md) at the project root.
 *
 ...
 *
 * NEVER throws: the routing doc is an advisory nicety — a broken LOOMTIDE.md write must
 ...
 * health by re-reading LOOMTIDE.md (the honest check, like the tarball sha) — a record
...
function syncRoutingDoc(project: string, dryRun: boolean): RoutingDocRecord {
  ...
  try {
    ...
    if (!existsSync(docPath)) {
      ...
      console.log(`     add to your CLAUDE.md/AGENTS.md: "Read LOOMTIDE.md before Unity work — it routes game-build/verify/perf tasks to Loomtide tools."`);
    ...
    if (onDiskVersion === null) {
      ...
      console.log(`  !! ${ROUTING_DOC_RELPATH} exists WITHOUT the Loomtide marker — leaving it untouched (not clobbering your file).`);
      console.log(`     If you want the generated routing doc, move it aside and re-run: loomtide install-bridge --project ${project}`);
    ...
    console.log(`     the bridge install continues; fix the path and re-run: loomtide install-bridge --project ${project}`);
...
async function installTarball(a: InstallArgs): Promise<number> {
  ...
  // 4. The agent-routing front door (LOOMTIDE.md) — install the workflow, not just the wiring.
  ...
  console.log(`    loomtide doctor --project ${a.project}`);
  return 0;
}
...
async function installEmbedded(a: InstallArgs): Promise<number> {
  ...
  const tmp = mkdtempSync(path.join(tmpdir(), "loomtide-bridge-"));
  try {
...
function printUsage(): void {
  console.log(
    [
      "Usage: loomtide install-bridge --project <unity-project-dir> [options]",
      "",
      "Install the Loomtide Unity bridge into a consumer project without cloning the",
      "Loomtide repo. Default: add the bundled bridge .tgz as a file: immutable",
      "dependency in Packages/manifest.json (Tests/ self-exclude; PackageCache is",
      "read-only). Writes ProjectSettings/LoomtideInstall.json.",
      "",
      "Options:",
      ...
      "                        (also honors $LOOMTIDE_BRIDGE_TARBALL)",
...
/**
 * Perform an install for already-parsed args. Exported so `loomtide update` can
...
export async function installBridge(parsed: InstallArgs): Promise<number> {
  try {
    ...
    if (error instanceof UsageError) {
      console.error(`[loomtide install-bridge] ${error.message}`);
      return 2;
    }
    const message = error instanceof Error ? error.message : String(error);
    console.error(`[loomtide install-bridge] ${message}`);
    return 1;
  }
}
