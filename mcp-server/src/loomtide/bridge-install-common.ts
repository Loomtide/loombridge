...
export const PKG_ID = "com.loomtide.unitybridge";
 ...
 * sha256 of the exact bytes Loomtide wrote. Hand-edit detection is per-file: a mismatch
 ...
 * everyone's `loomtide update` behaves identically after `git pull`).
 *
...
/** The record written to `<project>/ProjectSettings/LoomtideInstall.json`. */
export interface InstallMetadata {
   ...
   * front door (LOOMTIDE.md). Consumed by nothing — `doctor` deliberately re-derives
...
export const METADATA_RELPATH = path.join("ProjectSettings", "LoomtideInstall.json");

/**
 * Locate the bridge tarball that ships with this CLI build. Resolution order:
 *   1. explicit override (`--tarball` / `$LOOMTIDE_BRIDGE_TARBALL`)
 *   2. `<mcp-server>/bridge/`  (npm-packaged layout — tarball bundled here)
 *   3. `<repo>/dist/bridge/`   (dev layout — `scripts/loomtide-pack-bridge.sh` output)
 * Within a directory, the newest `com.loomtide.unitybridge-*.tgz` wins. Returns
...
export function resolveBundledTarball(override?: string): string | null {
  const explicit = override || process.env.LOOMTIDE_BRIDGE_TARBALL;
  if (explicit) {
