/**
 * `loomtide minigame scan` — open a live Unity scene, propose role candidates,
...
export function printScanUsage(): void {
  console.log(
    [
      "Usage: loomtide minigame scan --scene <Assets/...unity> [--id <id>] [--output <path>]",
      "",
...
export async function runScan(argv: string[], root: string = process.cwd()): Promise<number> {
  ...
  if ("error" in parsed) {
    console.error(`[loomtide minigame] scan: ${parsed.error}.`);
    printScanUsage();
    return 2;
  }
  ...
  try {
    ...
    console.error(hint ? hint.join("\n") : `[loomtide minigame] scan: ${message(error)}`);
    return 1;
  }

  const target = parsed.output ? path.relative(root, parsed.output) : "stdout";
  console.error(
    `[loomtide minigame] scan: found ${result.candidates.controls.length} control(s), ` +
  ...
  console.error(
    `[loomtide minigame] visualProfile: ${result.visualProfile} (${aspectNote}); ` +
      `valid: ${validProfiles} — edit if wrong.`,
  );
  if (result.stateSignal) {
    const s = result.stateSignal;
    console.error(
      `[loomtide minigame] stateSignal: auto-detected ${s.locator}:${s.component}:${s.property} ` +
        `(${s.enumValues.length} phases) — record will phase-align automatically. Clear/edit it in the draft if wrong.`,
    );
  } else {
    console.error(
      "[loomtide minigame] stateSignal: none auto-detected. A multi-screen/gesture game should declare one " +
  ...
  if (nextLines.length > 0) {
    console.error(`[loomtide minigame] next: review/edit the draft, then:`);
    for (const line of nextLines) console.error(line);
  } else {
    console.error(
      "[loomtide minigame] next: review/edit the draft, then run `loomtide minigame next` for the exact next command " +
