 ...
 * acceptance) now lives under `loomtide/genre-packs/<genre>/`, and this registry binds to it there.
...
// At runtime this file is dist/loomtide/genre-registry.js, so the package root is two up. Genre
...
export interface GenrePackRegistration {
  ...
  /**
   * The genre's verify-first (`loomtide verify --profile <id>`) feel-grading entry, lazily loaded.
...
const REGISTRY: Record<string, GenrePackRegistration> = {
  "platformer-2d": {
    ...
    sliceTemplatePath: path.join(PKG_ROOT, "src", "loomtide", "genre-packs", "platformer-2d", "slices.json"),
  ...
  "2d-shooter": {
    id: "2d-shooter",
    acceptanceTemplatePath: path.join(PKG_ROOT, "src", "loomtide", "genre-packs", "2d-shooter", "acceptance.json"),
    sliceTemplatePath: path.join(PKG_ROOT, "src", "loomtide", "genre-packs", "2d-shooter", "slices.json"),
  ...
  "3d-shooter": {
    id: "3d-shooter",
    acceptanceTemplatePath: path.join(PKG_ROOT, "src", "loomtide", "genre-packs", "3d-shooter", "acceptance.json"),
    sliceTemplatePath: path.join(PKG_ROOT, "src", "loomtide", "genre-packs", "3d-shooter", "slices.json"),
...
export async function resolveFeelProfileModule(
  ...
  return {
    unknownMessage:
      fallbackMessage ??
      `[loomtide verify] unknown --profile "${profileId}". No registered genre ships feel profiles.`,
  };
}
