/**
 * `loomtide assets` — deterministic Asset Manifest approval helpers.
 *
 * This is the production-facing bridge from the draft manifest created by
 * `loomtide plan --asset-mode ...` to an approved, provenance-complete
 * `.loomtide/ASSET_MANIFEST.json`. It deliberately consumes explicit JSON
...
import { loomtidePaths } from "./state.js";
...
function printUsage(): void {
  console.log(
    [
      "Usage: loomtide assets <registry-plan|registry-apply|generated-plan|generated-apply|pack-ingest|cover-build|discover> [options]",
      "",
      "Registry:",
      ...
      "  --from-selection accepts asset-web selection.json v1/v2; asset items role-match first and primitive-fallback second. Pack items require --catalog-api and are imported to .loomtide/registry/<packId>.json.",
...
function parseArgs(args: string[]): ParsedArgs {
  ...
  if (!["registry-plan", "registry-apply", "generated-plan", "generated-apply", "pack-ingest", "cover-build", "discover"].includes(first)) {
    console.error(`[loomtide assets] unknown action "${first}".`);
  ...
  for (let i = 1; i < args.length; i += 1) {
    ...
    if (arg === "--root") i += 1;
      ...
      console.error(`[loomtide assets] unknown option "${arg}".`);
...
async function readProjectManifest(root: string): Promise<AssetManifest> {
  ...
  if (!manifest) {
    throw new Error("No .loomtide/ASSET_MANIFEST.json — run `loomtide plan --asset-mode <mode>` first.");
  }
  return manifest;
}
...
function webSelectionItemsFromJson(value: unknown): WebSelectionItems {
  ...
  if (input.kind !== "loomtide-asset-selection") {
    throw new Error("--from-selection kind must be 'loomtide-asset-selection'.");
  }
...
function buildSelectionsFromWebSelection(args: {
  ...
  console.error(
    `[loomtide assets] from-selection matched=${Object.keys(selections).length}; ` +
...
async function writeImportedRegistryPacks(root: string, packs: Array<{ packId: string; registryPack: ReturnType<typeof catalogRecordsToRegistryPack> }>): Promise<void> {
  ...
  const registryDir = path.join(loomtidePaths(root).dir, "registry");
  ...
  for (const pack of packs) {
    ...
    console.error(`[loomtide assets] imported pack ${pack.packId} -> .loomtide/registry/${pack.packId}.json (${pack.registryPack.entries.length} assets)`);
  }
}
...
async function runRegistryPlan(parsed: ParsedArgs, options: RunAssetsOptions = {}): Promise<number> {
  ...
  console.error(`[loomtide assets] registry-plan slots=${plan.slots.length} issues=${plan.issues.length}`);
  return plan.issues.length === 0 ? 0 : 1;
}

async function runRegistryApply(parsed: ParsedArgs, options: RunAssetsOptions = {}): Promise<number> {
  const paths = loomtidePaths(parsed.root);
  const manifest = await readProjectManifest(parsed.root);
  const profile = await loadAssetProfile(requireOption(parsed.profilePath, "--profile"));
  ...
  const matched = approved
    ...
    : 0;
  console.error(
    `[loomtide assets] registry-apply status=${approved?.status ?? manifest.status}; ` +
...
async function runGeneratedPlan(parsed: ParsedArgs): Promise<number> {
  ...
  console.error(`[loomtide assets] generated-plan slots=${plan.slots.length} issues=${plan.issues.length}`);
  return plan.issues.length === 0 ? 0 : 1;
}
...
async function runGeneratedApply(parsed: ParsedArgs): Promise<number> {
  ...
  await writeAssetManifest(paths, approved);
  console.error(
    `[loomtide assets] generated-apply status=${approved.status}; ` +
...
export async function run(args: string[], options: RunAssetsOptions = {}): Promise<number> {
  ...
  try {
    ...
    console.error(`[loomtide assets] fatal: ${error instanceof Error ? error.message : String(error)}`);
    return 1;
  }
}
