 ...
 * `~/.loomtide/projects/<id>/` — deliberately under the user's home, NOT cwd, so
...
/** The default workspace path for an id: `~/.loomtide/projects/<id>`. */
export function projectWorkspace(id: string): string {
  return path.resolve(os.homedir(), ".loomtide", "projects", id);
}
