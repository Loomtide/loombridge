/**
 * `loomtide adopt` — the build-then-verify on-ramp (RCL-P02).
 *
 ...
 *     later `loomtide verify` / `loomtide doneness` still REFUSES (Epic-0 refuse
...
import { assertValidAcceptanceContract, validateAcceptanceContract } from "../verification/validator.js";
...
import { promoteGenreContract } from "./genre-contract/promote.js";
...
const DISCLAIMER =
  "This acceptance contract is a PROPOSAL inferred from existing artifacts by `loomtide adopt`. " +
  "It is UNVERIFIED — a starting point for human review, not a verdict. `adopt` never produces a " +
  "green/approved state: there is no build run, no verdict, and `loomtide verify` / `loomtide doneness` " +
...
function proposeContract(args: {
    ...
    if (!("error" in resolved)) {
      ...
      const contract: AcceptanceContract = {
        ...
        source: {
          ...
          note: `PROPOSED by \`loomtide adopt\` from the design-doc bundle ${rel(args.root, args.docsPath)} — UNVERIFIED, needs human review.`,
        },
      };
      return {
        contract,
  ...
  const contract: AcceptanceContract = {
    ...
    source: {
      ...(template.source ?? {}),
      note:
        `PROPOSED by \`loomtide adopt\` from the ${args.genre} pack template` +
...
export async function runAdopt(args: AdoptArgs): Promise<number> {
  ...
  if (!args.force && (await fileExists(paths.acceptance))) {
    console.error(
      `[loomtide adopt] NOT ready — ${rel(args.root, paths.acceptance)} already exists. ` +
  ...
  for (const [flag, p] of [["--docs", args.docsPath], ["--scene", args.scenePath]] as const) {
    if (p && !fsSync.existsSync(p)) {
      console.error(`[loomtide adopt] ${flag} path not found: ${p}`);
      return 2;
    }
  }
  ...
  if ("error" in proposal) {
    console.error(`[loomtide adopt] ${proposal.error}`);
    return 2;
  }
  ...
  if (!validation.valid) {
    console.error("[loomtide adopt] could not propose a valid contract from the available signals:");
  ...
  const feelSpec = {
    ...
    source: "derived from ACCEPTANCE.json `feel` by `loomtide adopt` (PROPOSAL — unverified)",
    metrics: feel,
  };
  ...
  const report: AdoptionReport = {
    ...
    nextSteps: [
      "Review and edit .loomtide/ACCEPTANCE.json — it is an inferred PROPOSAL, not ground truth.",
      "Establish + approve the Design Target (annotated hero shot) via `loomtide design set/approve`.",
      "Run `loomtide plan` to scaffold the slice roadmap from the reviewed contract.",
      "Build, then `loomtide verify`, then `loomtide doneness` — these still refuse until verified-green.",
    ],
  };
  ...
  console.error(`[loomtide adopt] genre=${proposal.genre} engine=${args.engine} root=${args.root}`);
  console.error(
    `[loomtide adopt] proposed (UNVERIFIED) contract from ${proposal.seededFrom}: ` +
      `${rel(args.root, paths.acceptance)}, ${rel(args.root, paths.feelSpec)}, ${rel(args.root, paths.gameSpec)}, ${rel(args.root, reportPath)}`,
  );
  for (const s of signals) {
    console.error(`[loomtide adopt] signal ${s.kind}: ${s.present ? "present" : "MISSING"}${s.detail ? ` — ${s.detail}` : ""}`);
  }
  ...
  if ("genreContract" in proposal && proposal.genreContract) {
    for (const line of formatDesignHardeningAdvisory(proposal.genreContract)) {
      console.error(`[loomtide adopt] ${line}`);
    }
  }
  console.error(`[loomtide adopt] ${DISCLAIMER}`);
  console.error("[loomtide adopt] next: review the proposal, then `loomtide design` → `loomtide plan` → build → verify → doneness.");
  return 0;
}
...
function parseArgs(args: string[]): ParsedAdoptArgs | { help: true } {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--genre") { genre = args[(i += 1)] ?? genre; genreExplicit = true; }
      ...
      console.error(`[loomtide adopt] unknown argument "${arg}".`);
      return { help: true };
    }
  }
  ...
  if (!genreExplicit) {
    if (!docsPath) {
      console.error(
        "[loomtide adopt] --genre is required (or pass --docs with a structured brief that declares its genre). " +
...
function printUsage(): void {
  console.log(
    [
      ...
      "NO verdict — `loomtide verify` / `loomtide doneness` still refuse until the",
      ...
      "  --force         Replace an existing .loomtide/ACCEPTANCE.json.",
...
function resolveEngine(parsed: ParsedAdoptArgs): { engine: string } | { exitCode: number } {
  if (parsed.engine !== undefined) {
    if (parsed.engine !== "unity") {
      console.error(`[loomtide adopt] --engine ${parsed.engine} is not supported; Loomtide currently supports Unity only`);
      return { exitCode: 2 };
    }
  ...
  if (detected.engine === null) {
    console.error(`[loomtide adopt] ${detected.reason}`);
    return { exitCode: 2 };
  }
  if (detected.engine !== "unity") {
    console.error(`[loomtide adopt] detected ${detected.engine}, but Loomtide currently supports Unity only`);
    return { exitCode: 2 };
  }
  console.error(`[loomtide adopt] detected engine: ${detected.engine}`);
