/**
 * `loomtide tuning-report` — the tuning analysis report verb (dogfood backlog High #6;
 * plan §9 "Tuning Needs Lever Isolation").
 *
 *   loomtide tuning-report --genre <id>|--schema <path> --runs <dir> [--out <p>] [--out-text <p>]
 *   loomtide tuning-report --genre <id>|--schema <path> --before <dir> --after <dir> \
...
function usage(): string {
  return [
    "loomtide tuning-report — deterministic tuning analysis over telemetry run-sets",
    "",
...
async function readPersonaContract(
  ...
  for (const w of parsed.warnings) {
    console.warn(`[loomtide tuning-report] persona contract warning: ${w}`);
  }
...
async function emitPersonaReport(
  ...
  for (const report of reports) {
    for (const caveat of report.caveats) {
      console.warn(`[loomtide tuning-report] persona caveat [${report.cohortLabel}]: ${caveat}`);
    }
  }
}
...
export async function run(argv: string[]): Promise<number> {
  ...
  if ("error" in parsed) {
    console.error(`[loomtide tuning-report] ${parsed.error}`);
    console.error(usage());
    return 2;
  }
  if (parsed.help) {
  ...
  if (single && pair) {
    console.error("[loomtide tuning-report] use --runs (single set) OR --before/--after (comparison), not both");
    return 2;
  }
  if (!single && !pair) {
    console.error("[loomtide tuning-report] provide --runs <dir>, or --before <dir> --after <dir>");
    console.error(usage());
    return 2;
  }
  if (pair && !(parsed.before && parsed.after)) {
    console.error("[loomtide tuning-report] a comparison needs BOTH --before <dir> and --after <dir>");
    return 2;
  }

  const schemaResult = await readSchema(parsed);
  if ("error" in schemaResult) {
    console.error(`[loomtide tuning-report] ${schemaResult.error}`);
    return 2;
  }
  ...
  if (personaResult && "error" in personaResult) {
    console.error(`[loomtide tuning-report] ${personaResult.error}`);
    return 2;
  }
  ...
  try {
    ...
    console.error(`[loomtide tuning-report] ${(e as Error).message}`);
    return 2;
  }
  ...
  try {
    if (single) {
      ...
      if (set.runs.length === 0) {
        console.error(`[loomtide tuning-report] no valid runs in ${parsed.runs} (${set.rejected.length} rejected)`);
    ...
    if (before.runs.length === 0 || after.runs.length === 0) {
      console.error(`[loomtide tuning-report] both sets need at least one valid run (before=${before.runs.length}, after=${after.runs.length})`);
...
async function warnOnClobber(p: string): Promise<void> {
  try {
    await fs.access(p);
    console.warn(`[loomtide tuning-report] overwriting existing ${p}`);
  } catch {
...
async function emit(report: ReturnType<typeof buildTuningReport>, args: Args): Promise<void> {
  ...
  if (report.leverIsolation?.warning) {
    console.warn(`\n[loomtide tuning-report] ${report.leverIsolation.warning}`);
  }
}
