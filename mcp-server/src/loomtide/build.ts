/**
 * `loomtide build` — the M1 verb-entry that mints the §3a supervisor block.
 *
 ...
 * `commands/loomtide/build.md` (not yet shipped) and never in this file (plan
 * §3b: no judgment in the CLI).
 *
 * Without this command, the §3a supervisor substrate is inert: `loomtide
...
import type { CapturePackSection } from "../verification/types.js";
import { gateInputFiles } from "../verification/run-gates.js";
import {
  getSliceVerdictPath,
  markDependentStale,
  nextUnblockedSlice,
...
export function evaluateBuildReadiness(opts: {
  ...
  if (!opts.hasContract) {
    blockers.push(
      "no .loomtide/ACCEPTANCE.json — run `loomtide plan` first (nothing to build toward).",
    );
  }
  ...
  if (opts.artDeferred) {
    warnings.push(
      "art:deferred (gray-box / feel-only) — Design Target + Asset Manifest gates are N/A; the run stays GROUNDED and `loomtide doneness` certifies on feel bands + structural assertions alone (RCL-D01).",
    );
  ...
  if (!opts.designReady) {
    ...
    if (opts.allowUngroundedPrototype) {
      warnings.push(
        `${cause}; --allow-ungrounded-prototype: the build run will be tagged \`ungrounded\` and \`loomtide doneness\` will refuse to certify it.`,
      );
    } else {
      blockers.push(
        `${cause}. Establish/re-approve via \`loomtide design\` (use --allow-ungrounded-prototype only for throwaway prototypes).`,
      );
    }
  }
  ...
  if (!opts.assetManifestReady) {
    ...
    if (opts.allowUngroundedPrototype) {
      ...
      blockers.push(`${cause}. Run \`loomtide plan --asset-mode <registry|generated|hybrid>\`, approve assets via \`loomtide assets\`, then build.`);
    }
  }
...
async function assetManifestReadiness(paths: ReturnType<typeof loomtidePaths>): Promise<{ ready: boolean; reason?: string }> {
  try {
...
export function deriveCaptureManifest(pack: CapturePackSection | undefined): string[] {
  ...
  for (const stateEntry of pack.states) {
    for (const cap of stateEntry.requiredCaptures) {
      ...
      // cannot mint a manifest entry that escapes `.loomtide/verify/`.
...
/** Derive capture manifest entries relative to `.loomtide/verify/` for a slice. */
...
export async function runBuild(args: BuildArgs): Promise<number> {
  ...
  if (declaredArtMode === "deferred" && design.status === "approved") {
    console.error(
      "[loomtide build] BLOCKED: acceptance contract declares `art.mode:\"deferred\"` (gray-box) but an APPROVED Design Target exists on disk — mutually exclusive. Remove the Design Target to stay gray-box, or set `art.mode:\"final\"` and build against the frozen hero shot (RCL-D01).",
    );
    return 1;
  }
  ...
  if (args.intent) console.error(`[loomtide build] intent: "${args.intent}"`);
  for (const w of readiness.warnings) console.error(`[loomtide build] WARNING: ${w}`);
  if (!readiness.ok) {
    for (const b of readiness.blockers) console.error(`[loomtide build] BLOCKED: ${b}`);
    return 1;
  }
  ...
  console.error(
    `[loomtide build] minted runId=${currentBuild.runId} startedAt=${currentBuild.startedAt}` +
      (ungrounded ? " (UNGROUNDED — disqualified from done)" : ""),
  );
  console.error(
    `[loomtide build] phase=built-unverified, captureManifest=${captureManifest.length} entries from capturePack` +
      (captureManifest.length === 0 ? " (none — contract has no capturePack)" : ""),
  );
  console.error(
    "[loomtide build] next: agent constructs via Loomtide MCP, saves captures to .loomtide/verify/<state>/,",
  );
  console.error(
    "[loomtide build]       then `loomtide verify --strict` and `loomtide doneness` to certify (§3a).",
  );
  return 0;
}
...
async function runSliceBuild(args: BuildArgs, plan: SlicePlan, ungrounded: boolean): Promise<number> {
  ...
  if (!slice) {
    console.error(
      "[loomtide build] no slice to build — a prior slice may be awaiting approval; " +
        "run `loomtide plan` to enter the approval flow, or all slices are approved.",
    );
    return 1;
  }
  ...
  console.error(
    `[loomtide build] slice ${slice.id} built (proof minted) runId=${runId} startedAt=${startedAt}` +
      (ungrounded ? " (UNGROUNDED — disqualified from done)" : ""),
  );
  console.error(
    `[loomtide build] captureManifest=${captureManifest.length} entries under .loomtide/verify/${slice.id}/` +
      (captureManifest.length === 0 ? " (none — slice gates have no captured-op file)" : ""),
  );
  console.error(
    `[loomtide build] next: construct slice ${slice.id}, save captures, then \`loomtide verify --slice ${slice.id}\`.`,
  );
  return 0;
}
...
function parseArgs(args: string[]): BuildArgs | { help: true } {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--root") root = path.resolve(args[(i += 1)] ?? root);
      ...
      console.error(`[loomtide build] unknown option "${arg}".`);
      return { help: true };
    } else {
...
function printUsage(): void {
  console.log(
    [
      'Usage: loomtide build ["<intent>"] [options]',
      "",
      "Mint a §3a build run + gate preconditions. The agent then constructs through",
      "the Loomtide MCP, saves captures to .loomtide/verify/<state>/, runs",
      "`loomtide verify --strict` and `loomtide doneness` to certify.",
      "",
      "Intent routing is the M2 agent-prose layer (commands/loomtide/build.md);",
...
export async function run(args: string[]): Promise<number> {
  ...
  try {
    ...
    console.error(`[loomtide build] fatal: ${message}`);
    return 1;
  }
}
