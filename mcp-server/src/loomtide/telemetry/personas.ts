...
// .json into dist). PKG_ROOT is three levels up from dist/loomtide/telemetry/personas.js.
const PKG_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..", "..", "..");

/**
 * Resolve the on-disk path to a genre pack's persona contract artifact
 * (`src/loomtide/genre-packs/<genre>/personas.json`). Pure path construction — does NOT
...
export function personaContractPathForGenre(genreId: string): string | null {
  ...
  return path.join(PKG_ROOT, "src", "loomtide", "genre-packs", genreId, "personas.json");
}
