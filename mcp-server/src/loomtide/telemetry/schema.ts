...
// At runtime this compiles to dist/loomtide/telemetry/schema.js, so the package root
 ...
 * (`src/loomtide/genre-packs/<genre>/telemetry.json`). Pure path construction — does
...
export function telemetrySchemaPathForGenre(genreId: string): string | null {
  ...
  return path.join(PKG_ROOT, "src", "loomtide", "genre-packs", genreId, "telemetry.json");
}
