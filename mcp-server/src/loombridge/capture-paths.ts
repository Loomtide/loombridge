 ...
 * `.loombridge/verify/`. Without constraint, a contract could declare
 ...
 *   2. build (`loombridge/build.ts`) — defense in depth at mint time; throws if
 *      `deriveCaptureManifest` projects an unsafe path (catches a hand-
 *      constructed pack that bypassed the validator).
 *   3. doneness (`loombridge/doneness.ts`) — refuses to certify when an entry
 *      in the minted manifest fails the check or its resolved path escapes
 *      `.loombridge/verify/`. Caught at certification time, not just write time.
 */
