...
RCL-F01/RCL-F02). It records exactly what Loombridge can measure for a top-down 3D arena today
...
half-life 0.10–0.18s, look-ahead 2–4m" — and **nothing in Loombridge could measure it**. This seed
...
the instantiated `.loombridge/SLICES.json`), so it survives instantiation and stays schema-valid.
...
- **GRL-C09** — mobile-touch feel (rigid vs floating joystick) is driven by a real-device feedback
  loop Loombridge has no in-editor proxy for; device-in-the-loop is entirely the human. Candidate for a
  `3d-topdown-mobile` pack + product-seam (document the human device loop).
- **GRL-C10** — editor perf ≠ device perf, with no Loombridge device-perf capture; ship a PerfHud
