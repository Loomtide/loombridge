 ...
 * (`src/loombridge/genre-packs/<genre>/personas.json`). Pure path construction — does NOT
...
export function personaContractPathForGenre(genreId: string): string | null {
  ...
  return path.join(PKG_ROOT, "src", "loombridge", "genre-packs", genreId, "personas.json");
}
