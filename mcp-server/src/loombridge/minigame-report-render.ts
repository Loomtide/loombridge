...
/** Escape a plain string, then turn its backtick spans into <code> (so `loombridge verify` reads as code). */
 ...
 * `loombridge minigame declare-background --id <id> <paths…>`. Default = container globs; unchecking a
...
function renderDeclarePanel(report: MinigameVerifyReport, scene?: string): string {
  ...
  const defaultCmd = confident.length
    ? `loombridge minigame declare-background --id ${id} ${confident.map((g) => `"${g.container}"`).join(" ")}`
...
<div class="topbar">
  <div class="brand">
    ...
    <span>Loombridge <span class="sub">Release Check</span></span>
  </div>
  <span class="cmd">loombridge verify --minigame</span>
</div>
...
${nextHtml}
${galleryHtml}
<footer>Generated by <code>loombridge verify --minigame</code>. Full detail in the accompanying <code>.json</code> report.</footer>
</div>
...
<script>
  ...
  function buildDeclCmd(panel){
    ...
    return 'loombridge minigame declare-background --id ' + id + ' ' + paths.map(function(p){ return '"' + p + '"'; }).join(' ');
  }
