...
test("GRADE-TIME VERIFICATION LITMUS: an over-cap manifest is a HARNESS FAULT, never a default fallback", async () => {
  ...
  try {
    ...
    // "Ungraded" is RECORDED as `not-compared`, never left absent: an absent field is what
    // made a refused pixel gate look, in the report, exactly like one that had nothing to do.
    assert.equal(capture.visualStatus, "not-compared", "the capture itself is ungraded, not unreadable");
...
test("GRADE-TIME VERIFICATION: a TAMPERED baseline frame is a harness fault at grade time too", async () => {
  ...
  try {
    ...
    // Same deliberate change as above: artifact-level fault, capture left ungraded and SAYING SO.
