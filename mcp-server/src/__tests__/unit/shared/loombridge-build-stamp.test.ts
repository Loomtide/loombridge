...
async function fakeModule(opts: {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "loombridge-stamp-"));
  const distDir = path.join(dir, "dist");
  const modDir = path.join(distDir, "loombridge");
...
test("resolveBuildStamp (running build): every field present and non-empty, never omitted", () => {
  ...
  assert.equal(stamp.tool, "loombridge");
...
test("a clean stamped build resolves version, commit (+dirty marker) and builtAt", async () => {
  ...
  // The one-line form matches `loombridge --version`.
  assert.equal(
    formatBuildStamp(stamp),
    "loombridge 0.1.0 (86bb72b+dirty, built 2026-06-12T11:46:20.722Z)",
  );
});
...
test("no build-info.json -> 'dev' stampStatus with a reason note, fields never omitted", async () => {
  ...
  assert.equal(formatBuildStamp(stamp), "loombridge 0.1.0 (dev)");
});
