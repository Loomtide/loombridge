...
import { exitCodeForLiveProfileCapture, run as runVerifyCli } from "../loomtide/verify.js";
import {
  buildProfileReport,
  evaluateProfile,
  runVerifyProfile,
} from "../loomtide/genre-packs/platformer-2d/verify-profile.js";
import { parseMeasurements } from "../loomtide/genre-packs/platformer-2d/measurements.js";
...
async function tmpRoot(unity = true): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "loomtide-vp-"));
  if (unity) {
...
test("profile mode runs standalone with no ACCEPTANCE.json/SLICES.json", async () => {
  ...
  assert.equal(await exists(path.join(root, ".loomtide", "reports", "feel-profile.json")), false);
});

test("profile report carries a producedBy build stamp (F5 #14 — audit provenance)", async () => {
  // The report must record which loomtide build graded it; absence is impossible
  ...
  const report = await readReport(workspace);
  assert.equal(report.producedBy.tool, "loomtide");
...
test("non-mutation: profile mode writes report siblings, never contract/slices/state", async () => {
  ...
  const dot = path.join(root, ".loomtide");
...
test("--setup-capture previews by default and writes a generic capture contract with --apply", async () => {
  ...
  assert.equal(await exists(path.join(root, ".loomtide", "reports", "feel-profile.json")), false);
  assert.equal(await exists(path.join(root, ".loomtide", "feel", "capture-contract.json")), false);
});

test("--setup-capture --id defaults to ~/.loomtide/projects/<id>/feel/capture-contract.json", async () => {
  const root = await tmpRoot();
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "loomtide-vp-home-"));
  ...
  try {
    ...
    assert.equal(code, 0);
    assert.ok(await exists(path.join(home, ".loomtide", "projects", "external-game", "feel", "capture-contract.json")));
    assert.equal(await exists(path.join(root, ".loomtide", "feel", "capture-contract.json")), false);
  } finally {
...
test("profile mode REFUSES an underivable workspace id instead of silently inventing one", async () => {
  ...
  const parent = await fs.mkdtemp(path.join(os.tmpdir(), "loomtide-vp-"));
  const root = path.join(parent, "123");
  await fs.mkdir(root, { recursive: true });
  const code = await runVerifyCli(["--profile", "precision", "--root", root]);
  assert.equal(code, 2); // asks for --id; never falls back to ~/.loomtide/projects/project
});
...
test("a DETECTED non-Unity engine (godot) never reports a green pass, even all-in-band", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "loomtide-vp-godot-"));
...
test("profile mode honors an explicit --output path (not the contract build-verdict.json default)", async () => {
  ...
  assert.equal(await exists(path.join(root, ".loomtide", "reports", "build-verdict.json")), false);
});
...
test("S5d: generic feel-capture provenance re-derives as verified", async () => {
  ...
  await fs.writeFile(
    ...
    JSON.stringify({
      metrics: { runSpeed: 14 },
      provenance: {
        note: "Generated by Loomtide generic feel capture runner.",
        sources: [
          {
