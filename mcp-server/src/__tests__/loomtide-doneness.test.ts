...
import { validateAssetManifest } from "../loomtide/asset-manifest.js";
import { designPaths, designStatus, setDesignTarget } from "../loomtide/design.js";
...
import { createDraftAssetManifest } from "../loomtide/asset-manifest.js";
import {
...
async function tmpRoot(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "loomtide-done-"));
}
...
function baselineState(): LoomtideState {
  return {
    genre: "platformer-2d",
    engine: "unity",
...
test("checkAssetSourceFidelity — an approved 3d-shooter manifest passes the asset-source doneness chain", async () => {
  ...
  const draft = createDraftAssetManifest({
    mode: "registry",
    heroShot: { path: ".loomtide/design/hero-shot.png", sha256: FROZEN_SHA },
...
test("isSliceDone — ok for verified slice with bound fresh pass verdict and captures", async () => {
  ...
  try {
    ...
    const r = await isSliceDone(slice, loomtidePaths(root));
    assert.equal(r.ok, true, r.reasons.join(" | "));
  } finally {
...
test("isSliceDone — refuses missing and mismatched verdict.runId", async () => {
  ...
  try {
    ...
    let r = await isSliceDone(slice, loomtidePaths(root));
    assert.equal(r.ok, false);
    ...
    r = await isSliceDone(slice, loomtidePaths(root));
    assert.equal(r.ok, false);
...
test("isSliceDone — refuses non-pass, missing capture, unsafe capture, and wrong state", async () => {
  ...
  try {
    ...
    await fs.rm(path.join(loomtidePaths(root).verifyInputs, "s1", "verify-manifest.json"));
    r = await isSliceDone(missing, loomtidePaths(root));
    assert.equal(r.ok, false);
    ...
    r = await isSliceDone(unsafe, loomtidePaths(root));
    assert.equal(r.ok, false);
    ...
    r = await isSliceDone(wrongState, loomtidePaths(root));
    assert.equal(r.ok, false);
...
test("slice doneness roll-up — all approved + done + dependency order exits 0", async () => {
  ...
  try {
    ...
    await writeSlicePlan(loomtidePaths(root), planOf([a, b]));
    assert.equal(await runDoneness({ root }), 0);
  } finally {
    await fs.rm(root, { recursive: true, force: true });
  }
...
test("slice doneness roll-up — an empty roadmap is NOT done (no 0/0 false-green)", async () => {
  const root = await tmpRoot();
  try {
    await writeSlicePlan(loomtidePaths(root), planOf([]));
    assert.equal(await runDoneness({ root }), 1);
  } finally {
    await fs.rm(root, { recursive: true, force: true });
  }
...
test("asset gate (RCL-D02) — primitiveFinal passes ONLY under art:deferred; inert under art:final", () => {
  const heroShot = { path: ".loomtide/design/hero-shot.png", sha256: "a".repeat(64) };
