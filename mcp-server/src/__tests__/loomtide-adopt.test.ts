/**
 * `loomtide adopt` — build-then-verify on-ramp (RCL-P02) + `plan --brief`
...
import { runAdopt, type AdoptionReport } from "../loomtide/adopt.js";
import { runDoneness } from "../loomtide/doneness.js";
import { loomtidePaths, readState, writeState, fileExists } from "../loomtide/state.js";
...
async function tmpRoot(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "loomtide-adopt-"));
}
...
test("adopt — emits a proposed ACCEPTANCE that validates but is marked proposal/unverified, and doneness still refuses", async () => {
  ...
  assert.match(String((contract.source as { note?: string })?.note ?? ""), /PROPOSED by `loomtide adopt`/);
...
test("adopt --force resets to UNVERIFIED — a forced re-adopt over a previously-VERIFIED state carries NO stale verdict (P2)", async () => {
  ...
  await writeState(paths, {
    ...
    lastVerdict: { status: "pass", at: "2026-06-01T00:00:00.000Z", verdictPath: ".loomtide/build-verdict.json" },
