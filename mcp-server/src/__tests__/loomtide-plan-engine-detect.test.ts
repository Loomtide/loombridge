...
import { detectEngine, run } from "../loomtide/plan.js";
...
async function tmpRoot(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "loomtide-engine-"));
}
...
test("run — engine omitted + Godot project → exit 2, Unity only", async () => {
  ...
  try {
    ...
    const { code, err } = await runCli(["--root", root]);
    assert.equal(code, 2);
    assert.match(err, /detected godot, but Loomtide currently supports Unity only/);
...
test("run — explicit --engine godot exits 2 and writes no scaffold", async () => {
  ...
  try {
    ...
    assert.equal(code, 2);
    assert.match(err, /--engine godot is not supported; Loomtide currently supports Unity only/);
