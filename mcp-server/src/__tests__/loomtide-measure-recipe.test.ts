...
import {
  ...
  tapMsToTicks,
  type InputMap,
} from "../loomtide/genre-packs/platformer-2d/measure-recipe.js";
import {
  assembleMeasurements,
  type MetricCapture,
} from "../loomtide/genre-packs/platformer-2d/assemble-measurements.js";
...
import { buildProfileReport } from "../loomtide/genre-packs/platformer-2d/verify-profile.js";
...
async function unityRoot(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "loomtide-s5c-"));
...
test("assembleMeasurements: a derived inputToSfxLatency lands in alsoMeasured and NEVER gates the verdict", async () => {
  ...
  const { loadProfile } = await import("../loomtide/genre-packs/platformer-2d/profiles.js");
...
test("no shipped profile bands a sync-family metric (measure-only invariant)", async () => {
  const { loadAllProfiles } = await import("../loomtide/genre-packs/platformer-2d/profiles.js");
  const { KNOWN_PROFILE_METRICS } = await import("../loomtide/genre-packs/platformer-2d/types.js");
