...
test("read/write asset manifest helpers round-trip .loomtide/ASSET_MANIFEST.json", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "loomtide-manifest-"));
  const paths = loomtidePaths(root);
  const value = manifest("hybrid");

  await writeAssetManifest(paths, value);

  const raw = await fs.readFile(path.join(root, ".loomtide", "ASSET_MANIFEST.json"), "utf-8");
