...
import { exitCodeForVerdict, runVerify } from "../loomtide/verify.js";
import { fileExists, loomtidePaths, readState } from "../loomtide/state.js";

async function tmpRoot(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "loomtide-cli-"));
}
...
test("plan scaffolds .loomtide/ for the platformer genre", async () => {
...
test("verify writes a verdict, updates STATE, and enforces strict on warn", async () => {
  ...
  // No captures in .loomtide/verify -> every gate degrades to WARN -> status warn.
