...
import { run, runAsk } from "../loomtide/ask.js";
...
async function tmpRoot(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "loomtide-ask-"));
}
...
test("ask is read-only on a fresh project and does not create .loomtide", async () => {
...
test("ask where are we mirrors compact progress", async () => {
  ...
  try {
    ...
    assert.match(output, /^Next: run \/loomtide:build or say continue\./m);
...
test("ask what next returns the status model next action", async () => {
  ...
  try {
    ...
    assert.match(output, /Next: framing needs capture\/verify evidence; run \/loomtide:build or say continue\./);
...
test("ask why approval explains approval without exposing --go", async () => {
  ...
  try {
    ...
    assert.match(output, /Next: say "approve framing" or run \/loomtide:plan to approve and advance\./);
...
test("ask --root without value is refused instead of falling back to cwd", async () => {
  ...
  assert.equal(code, 2);
  assert.match(output, /Usage: loomtide ask/);
});
...
test("CLI dispatcher routes ask", async () => {
  ...
  try {
    ...
    const { code, output } = await captureStdout(() =>
      loomtideCli(["node", "cli", "ask", "--root", root, "where", "are", "we"]),
    );
    assert.equal(code, 0);
