...
async function tmpRoot(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "loomtide-design-"));
}
...
test("designStatus reports missing on a fresh project", async () => {
  ...
  const report = await designStatus(loomtidePaths(root));
...
test("set --kind composition-reference is recorded; approve preserves the kind (no silent promotion)", async () => {
  ...
  assert.equal((await designStatus(loomtidePaths(root))).kind, "composition-reference");
});
...
test("plan hard-gates on Design Target by default (no flag needed)", async () => {
  ...
  assert.equal((await readState(loomtidePaths(root)))?.designTarget, "approved");
});
...
test("verify embeds the frozen Design Target reference in the verdict (advisory)", async () => {
  const root = await tmpRoot();
  // Scaffold .loomtide/ without an approved target — the test is about verify,
