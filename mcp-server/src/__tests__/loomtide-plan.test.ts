...
import { renderGameSpec, runPlan } from "../loomtide/plan.js";
import { nextActionFor, readState, loomtidePaths, type LoomtideState } from "../loomtide/state.js";
import { setDesignTarget } from "../loomtide/design.js";

async function tmpRoot(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "loomtide-plan-"));
}

async function namedTmpRoot(name: string): Promise<string> {
  const parent = await fs.mkdtemp(path.join(os.tmpdir(), "loomtide-plan-parent-"));
...
function stateWith(patch: Partial<LoomtideState>): LoomtideState {
  return {
    genre: "platformer-2d",
    engine: "unity",
    phase: "planned",
...
test("nextActionFor — planned without an approved target points at the Design Target", () => {
  ...
  assert.match(action, /loomtide design/);
});

test("nextActionFor — planned WITH approval points at build", () => {
  assert.match(nextActionFor(stateWith({ phase: "planned", designTarget: "approved" })), /loomtide build/);
});
...
test("nextActionFor — verified-green points at doneness + the hero-shot review", () => {
  ...
  assert.match(action, /loomtide doneness/);
...
test("runPlan — --genre-contract refuses unregistered promoted genres", async () => {
  const root = await tmpRoot();
  try {
    const sourcePath = path.join(process.cwd(), "src", "loomtide", "genre-contract", "examples", "2d-shooter.contract.json");
...
test("runPlan — --genre-contract refuses to mix with existing artifacts without --force", async () => {
  ...
  try {
    await runPlan({ root, genre: "platformer-2d", engine: "unity", force: false, allowMissingDesignTarget: true });
    const genreContractPath = path.join(process.cwd(), "src", "loomtide", "genre-contract", "examples", "2d-shooter.contract.json");
    const code = await runPlan({
      root,
      genre: "2d-shooter",
      genreContractPath,
    ...
    assert.equal(code, 2);
    const state = await readState(loomtidePaths(root));
