...
} from "../loomtide/genre-packs/platformer-2d/reachability-profile.js";
import {
  buildProfileReport,
...
import { run as runVerifyCli } from "../loomtide/verify.js";
import type {
  PlatformerFeelProfile,
  ProfileMetricTarget,
} from "../loomtide/genre-packs/platformer-2d/types.js";
...
test("F4 CLI: --profile with a reachable --layout passes the gate; report carries reachability.pass", async () => {
  ...
  try {
    ...
    assert.equal(await fileExists(path.join(root, ".loomtide", "reports", "feel-profile.json")), false);
  } finally {
    await fs.rm(root, { recursive: true, force: true });
  }
});
