...
import {
  ...
  mintRunId,
  runBuild,
} from "../loomtide/build.js";
import { HERO_SHOT_FIDELITY_CRITERIA, runDoneness } from "../loomtide/doneness.js";
...
async function tmpRoot(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "loomtide-build-"));
}
...
test("build blocks before plan (no contract, no state mutation)", async () => {
  ...
  assert.equal(await readState(loomtidePaths(root)), null);
});
...
test("build auto-detects SLICES.json and mints proof for nextUnblockedSlice", async () => {
  ...
  assert.equal(slice.proof?.verdictPath, path.join(".loomtide", "reports", "slices", "framing.verdict.json"));
...
test("end-to-end: plan → build → verify → doneness lights up the §3a supervisor", async () => {
  ...
  const { updateState } = await import("../loomtide/state.js");
...
test("build SLICE-mode tags currentBuild ungrounded when escaping the unapproved-asset gate (PR #349 finding 2, slice path)", async () => {
  ...
  const genreContractPath = path.join(
    process.cwd(), "src", "loomtide", "genre-contract", "examples", "2d-shooter.contract.json",
  );
