/**
 * `loomtide verify --slice <id>` (S2a) — data-driven gate selection from the
 * slice's `acceptance.gates` + a PER-SLICE verdict at
 * `.loomtide/reports/slices/<id>.verdict.json`.
 *
 * The harness mirrors verification-run-gates.test.ts: write the gate-input JSON
 * captures into a tmp `.loomtide/verify/<id>/`, write a SLICES.json + an
...
async function tmpRoot(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "loomtide-verify-slice-"));
}
...
/** Scaffold .loomtide/ + copy the real acceptance contract into place. */
...
test("verify --slice fresh pass commits built -> verified and records a checkpoint", async () => {
  ...
  try {
    ...
    assert.equal(await fileExists(path.join(getSliceFixtureDir(paths, "s1-hud"), "loomtide", "SLICES.json")), true);
  } finally {
    await fs.rm(root, { recursive: true, force: true });
  }
});
...
test("verify --slice with no SLICES.json → exit 2 with the scaffold message", async () => {
  const root = await tmpRoot();
  try {
    await scaffold(root); // .loomtide/ exists but NO SLICES.json
    const code = await runVerify({ ...baseArgs(root), slice: "s1-hud" });
    assert.equal(code, 2);
  } finally {
    await fs.rm(root, { recursive: true, force: true });
...
test("verify --slice — an unsafe slice id cannot even be persisted (rejected at rest)", async () => {
  ...
  try {
    ...
    // backstop — covered by a unit test in loomtide-slices.test.ts.
...
test("verify --inputs override survives a --slice run (explicit beats the per-slice default)", async () => {
  ...
  try {
    ...
    const paths = loomtidePaths(root);
    await writeSlicePlan(paths, planWith(["manifest", "ui-conformance"]));
    // Put the captures somewhere OTHER than .loomtide/verify/s1-hud and point
