...
const GEN_URL = pathToFileURL(path.join(REPO_ROOT, "scripts", "sync-loomtide-artifacts.mjs")).href;
...
test("generator plans a wrapper per command and no project-local config.toml", async () => {
  ...
  const wrappers = artifacts.filter((a) => a.relPath.startsWith(`scripts${path.sep}loomtide-`));
...
test("generated wrappers satisfy the shim-parity invariants (same md, codex exec --ephemeral, build threads $*)", async () => {
  ...
  for (const art of plannedArtifacts()) {
    if (!art.relPath.startsWith(`scripts${path.sep}loomtide-`)) continue;
    const name = path.basename(art.relPath).replace(/^loomtide-/, "");
    assert.match(art.content, new RegExp(`commands/loomtide/${name}\\.md`), `${name}: references its .md`);
...
test("staleWrappers flags an orphaned generated wrapper but not the .sh helpers or current commands", async () => {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "loomtide-stale-"));
  try {
    // Current command wrappers + a stale one + the hand-written .sh helpers.
    for (const f of [
      "loomtide-plan",
      "loomtide-build",
      "loomtide-oldcommand", // removed from plugin.json → stale
      "loomtide-embed-bridge.sh", // helper, not generated → keep
      "loomtide-install-locally.sh",
      "loomtide-checkpoint.sh",
    ]) {
      await fs.writeFile(path.join(dir, f), "x", "utf-8");
    }
    const stale = staleWrappers(dir, ["plan", "build", "verify", "e2e"]);
    assert.deepEqual(stale, ["loomtide-oldcommand"], "only the orphaned extensionless wrapper is stale");
  } finally {
...
test("ANTI-DRIFT: committed shim artifacts match the generator (run the generator + commit if this fails)", async () => {
  ...
  for (const art of plannedArtifacts()) {
    ...
    assert.equal(
      onDisk,
      art.content,
      `${art.relPath} has drifted from the generator — run \`node scripts/sync-loomtide-artifacts.mjs\` and commit.`,
    );
  }
});
...
test("NO DUAL SOURCE: the generator's doc resolver matches resolveStartingParams for every shipped profile", async () => {
  ...
  const { resolveStartingParams } = await import("../loomtide/genre-packs/platformer-2d/solve-params.js");
  const { loadProfile, SHIPPED_PROFILE_IDS } = await import("../loomtide/genre-packs/platformer-2d/profiles.js");
