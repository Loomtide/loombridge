...
import { run as runAssets } from "../loomtide/assets.js";
import { readAssetManifest, type AssetManifest, type RequiredAssetRole } from "../loomtide/asset-manifest.js";
...
async function tmpRoot(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "loomtide-assets-"));
}
...
async function draftManifest(root: string, mode: "registry" | "generated"): Promise<AssetManifest> {
  ...
  const manifest = await readAssetManifest(loomtidePaths(root));
  assert.ok(manifest);
...
async function webSelectionForPlan(root: string, overrides: Record<string, Partial<{ role: string; primitive: string; registryId: string }>> = {}): Promise<{
  ...
  const selectionPath = path.join(root, ".loomtide/reports/web-selection.json");
...
test("loomtide assets registry-plan/apply approves the manifest without hand-editing JSON", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const registryPath = path.join(repoRoot, "asset-layer/registry/platformer-2d.json");
  const profilePath = path.join(repoRoot, "asset-layer/profiles/2d-platformer.json");
  const planPath = path.join(root, ".loomtide/reports/registry-plan.json");
  const selectionsPath = path.join(root, ".loomtide/reports/registry-selections.json");
  ...
  const approved = await readAssetManifest(loomtidePaths(root));
...
test("loomtide assets registry-plan/apply approves a 3d-shooter manifest with source-bound model paths", async () => {
  const root = await tmpRoot();
...
test("loomtide assets registry-plan/apply can use a hosted catalog source instead of a registry pack", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const registryPath = path.join(repoRoot, "asset-layer/registry/platformer-2d.json");
  const profilePath = path.join(repoRoot, "asset-layer/profiles/2d-platformer.json");
  const catalogPath = path.join(root, "catalog.json");
  const planPath = path.join(root, ".loomtide/reports/catalog-plan.json");
  const selectionsPath = path.join(root, ".loomtide/reports/catalog-selections.json");
  const registry = await loadRegistryPack(registryPath);
  const catalog = {
    schemaVersion: "1",
...
test("loomtide assets registry-apply --from-selection maps role-keyed web selections", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
...
test("loomtide assets registry-apply --from-selection --catalog-api resolves exact selection ids", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
...
test("loomtide assets registry-apply --from-selection accepts schema v2 asset-only selections", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const { registryPath, profilePath, selectionPath, items } = await webSelectionForPlan(root);
  await writeJson(selectionPath, {
...
test("loomtide assets registry-apply --from-selection imports schema v2 pack items with paged catalog assets idempotently", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const registryPath = path.join(repoRoot, "asset-layer/registry/platformer-2d.json");
  const profilePath = path.join(repoRoot, "asset-layer/profiles/2d-platformer.json");
  ...
  const registryPackPath = path.join(root, ".loomtide/registry", `${packId}.json`);
...
test("loomtide assets registry-apply --from-selection pack-only skips manifest slot fill", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const manifestPath = path.join(root, ".loomtide/ASSET_MANIFEST.json");
  ...
  const imported = JSON.parse(await fs.readFile(path.join(root, ".loomtide/registry", `${packId}.json`), "utf-8")) as { entries: unknown[] };
  assert.equal(imported.entries.length, 3);
});

test("loomtide assets registry-apply --from-selection mixed v2 applies asset selections and imports packs atomically", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const { registryPath, profilePath, selectionPath, items } = await webSelectionForPlan(root);
  const packId = "kenney.3d-assets.blaster-kit";
...
test("loomtide assets registry-apply --from-selection aborts mixed v2 without partial writes on invalid pack", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const manifestPath = path.join(root, ".loomtide/ASSET_MANIFEST.json");
  const before = await fs.readFile(manifestPath, "utf-8");
  ...
  assert.equal(await fs.readFile(manifestPath, "utf-8"), before);
  await assert.rejects(fs.stat(path.join(root, ".loomtide/registry", `${badPackId}.json`)), /ENOENT/);
});

test("loomtide assets registry-apply --from-selection rejects pack items under schema v1", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const before = await fs.readFile(path.join(root, ".loomtide/ASSET_MANIFEST.json"), "utf-8");
  const registryPath = path.join(repoRoot, "asset-layer/registry/platformer-2d.json");
  const profilePath = path.join(repoRoot, "asset-layer/profiles/2d-platformer.json");
  const selectionPath = path.join(root, ".loomtide/reports/web-selection.json");
  await writeJson(selectionPath, {
  ...
  assert.equal(await fs.readFile(path.join(root, ".loomtide/ASSET_MANIFEST.json"), "utf-8"), before);
  await assert.rejects(fs.stat(path.join(root, ".loomtide/registry")), /ENOENT/);
});

test("loomtide assets registry-apply --from-selection rejects invalid pack ids without writing", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const before = await fs.readFile(path.join(root, ".loomtide/ASSET_MANIFEST.json"), "utf-8");
  const registryPath = path.join(repoRoot, "asset-layer/registry/platformer-2d.json");
...
test("loomtide assets registry-apply --from-selection --catalog-api refuses unresolved selection ids without writing", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const before = await fs.readFile(path.join(root, ".loomtide/ASSET_MANIFEST.json"), "utf-8");
  const { registryPath, profilePath, selectionPath, items } = await webSelectionForPlan(root);
...
test("loomtide assets registry-apply --from-selection falls back from primitive role to manifest slot", async () => {
  const root = await tmpRoot();
...
test("loomtide assets registry-apply --from-selection refuses ambiguous primitive mappings without writing", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const before = await fs.readFile(path.join(root, ".loomtide/ASSET_MANIFEST.json"), "utf-8");
  const { registryPath, profilePath, selectionPath, items } = await webSelectionForPlan(root);
...
test("loomtide assets registry-apply --from-selection refuses unknown registryId", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const before = await fs.readFile(path.join(root, ".loomtide/ASSET_MANIFEST.json"), "utf-8");
  const { registryPath, profilePath, selectionPath, items } = await webSelectionForPlan(root);
...
test("loomtide assets registry-apply --from-selection rejects malformed selection JSON before writing", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const before = await fs.readFile(path.join(root, ".loomtide/ASSET_MANIFEST.json"), "utf-8");
  const registryPath = path.join(repoRoot, "asset-layer/registry/platformer-2d.json");
  ...
  assert.equal(result.code, 1);
  assert.match(result.err, /kind must be 'loomtide-asset-selection'/);
  assert.equal(await fs.readFile(path.join(root, ".loomtide/ASSET_MANIFEST.json"), "utf-8"), before);
});

test("loomtide assets registry-apply --from-selection --strict-roles refuses primitive-only unfilled slots", async () => {
  const root = await tmpRoot();
  const manifest = await draftManifest(root, "registry");
  const player = manifest.assets.find((asset) => asset.role === "player-character");
  assert.ok(player);
...
test("loomtide assets registry-apply --from-selection is deterministic for identical inputs", async () => {
  const root = await tmpRoot();
  await draftManifest(root, "registry");
  const manifestPath = path.join(root, ".loomtide/ASSET_MANIFEST.json");
  const before = await fs.readFile(manifestPath, "utf-8");
...
test("loomtide assets generated-plan/apply approves generated exports with provenance", async () => {
  const root = await tmpRoot();
  const manifest = await draftManifest(root, "generated");
  const annotationsPath = path.join(root, ".loomtide/reports/generated-annotations.json");
  const planPath = path.join(root, ".loomtide/reports/generated-plan.json");
  const exportsPath = path.join(root, ".loomtide/reports/generated-exports.json");
...
test("loomtide assets resolves path flags against --root even when --root is last", async () => {
  const root = await tmpRoot();
  const manifest = await draftManifest(root, "generated");
  const annotationsPath = path.join(root, ".loomtide/reports/generated-annotations.json");
  const outputRel = ".loomtide/reports/generated-plan-root-last.json";
  ...
  assert.equal(await runAssets([
    "generated-plan",
    "--annotations", ".loomtide/reports/generated-annotations.json",
    "--output", outputRel,
    "--root", root,
  ]), 0);
