...
import { knownGenreIds } from "../loomtide/genre-registry.js";
 ...
 *
 * SCOPE: the plan → build → verify → doneness genre pipeline = everything under `src/loomtide/` and
...
const SRC_ROOTS = ["src/loomtide", "src/verification"].map((rel) => path.join(process.cwd(), rel));
const EXCLUDED_FILE = path.join(process.cwd(), "src", "loomtide", "genre-registry.ts");
