...
import { resolveBriefBundle } from "../loomtide/brief-bundle.js";
import { loomtidePaths, fileExists } from "../loomtide/state.js";
...
async function tmpDir(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "loomtide-brief-"));
}
...
async function namedRoot(name: string): Promise<string> {
  const parent = await fs.mkdtemp(path.join(os.tmpdir(), "loomtide-brief-named-"));
...
test("plan --brief <bundle> — produces a contract equivalent to the --genre-contract interview path", async () => {
  ...
  const briefContract = await readJson<unknown>(loomtidePaths(briefRoot).acceptance);
  const directContract = await readJson<unknown>(loomtidePaths(contractRoot).acceptance);
  assert.deepEqual(briefContract, directContract, "brief-seeded contract equals the interview-seeded contract");

  const briefSlices = await readJson<unknown>(loomtidePaths(briefRoot).slices);
  const directSlices = await readJson<unknown>(loomtidePaths(contractRoot).slices);
...
test("plan --brief — a docs bundle with no structured brief is a clear error (exit 2), nothing written", async () => {
  ...
  assert.equal(code, 2);
  assert.equal(await fileExists(loomtidePaths(root).acceptance), false);
});
