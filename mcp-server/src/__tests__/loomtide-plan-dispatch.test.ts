...
import {
  ...
  writeSlicePlan,
  type SlicePlan,
} from "../loomtide/slices.js";
import { loomtidePaths, readState, updateState } from "../loomtide/state.js";

async function tmpRoot(): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), "loomtide-dispatch-"));
}
  ...
  return {
    ...
    source: {
      ...
      provenance: {
        ...
        origin: "fixture",
        fixture: "loomtide-plan-dispatch",
      },
    },
    provider: {
      name: "Fixture",
...
async function approveAssetManifest(root: string, mode: AssetManifestMode = "hybrid"): Promise<void> {
  ...
  const manifest = createDraftAssetManifest({
    mode,
    heroShot: { path: ".loomtide/design/hero-shot.png", sha256: pngSha256 },
  });
 ...
 * + dispatch lines all use console.error with the `[loomtide plan]` prefix).
 */
...
test("design mode — no design target: gate refuses, no roadmap written", async () => {
  ...
  try {
    ...
    assert.equal(await readSlicePlan(loomtidePaths(root)), null);
  } finally {
...
test("design mode — --allow-missing-design-target completes (0) but writes no roadmap", async () => {
  ...
  try {
    ...
    assert.equal(await readSlicePlan(loomtidePaths(root)), null, "no roadmap until design approved");
  } finally {
    await fs.rm(root, { recursive: true, force: true });
  }
});
...
test("scaffold — design approved + no roadmap + bare plan writes SLICES.json and announces first slice", async () => {
  ...
  try {
    ...
    assert.match(err, /next: review the slice above, then run `loomtide build`/);
    const plan = await readSlicePlan(loomtidePaths(root));
...
test("plan-slice mode — announces the next unblocked slice read-only, writes nothing new", async () => {
  ...
  try {
    ...
    assert.match(err, /Plan next slice: framing/);
    assert.match(err, /loomtide build/);
...
test("await-approval mode — echoes the approval seam, flips no state", async () => {
  ...
  try {
    ...
    assert.match(err, /Next: say "approve framing" or run \/loomtide:plan to approve and advance\./);
...
async function makeSliceDone(root: string, plan: SlicePlan, id: string): Promise<void> {
  ...
  slice.proof = {
    ...
    verdictPath: `.loomtide/reports/slices/${id}.verdict.json`,
...
test("await-approval mode — --go approves a verified+done slice and advances", async () => {
  ...
  try {
    ...
    assert.match(err, /Next: run \/loomtide:build or say continue\./);
  ...
  try {
    ...
    for (const slice of plan.slices.slice(0, -1)) {
      ...
      slice.proof = {
        runId: `run-${slice.id}`,
        startedAt: "2026-05-28T00:00:00.000Z",
        verdictPath: `.loomtide/reports/slices/${slice.id}.verdict.json`,
        captureManifest: [],
...
test("await-approval mode — --go --note --signoff copies durable artifact and records hash", async () => {
  ...
  try {
    ...
    assert.equal(framing.proof?.signoffArtifact, ".loomtide/reports/slices/framing/signoff.png");
...
test("slice template — loaded by the scaffold path passes assertValidSlicePlan", async () => {
  ...
  try {
    await approveDesignTarget(root);
    await runPlan({ ...base, root, go: true });
    const plan = (await readSlicePlan(loomtidePaths(root))) as SlicePlan;
    assert.doesNotThrow(() => assertValidSlicePlan(plan));
