/**
 * `loombridge verify --minigame --contract <path> --captures <dir>` (S6c-3).
 *
...
import type { FlowReport } from "./interaction-flow.js";
import { nextStepLinesFor } from "./minigame-next.js";
import { INPUT_RESPONSE_CHECK, type InputResponseReport } from "./input-response.js";
import {
  baselineThresholds,
  compareToBaseline,
  resolveBaselineRef,
...
function buildBaselineSection(
  ...
  const reApprove = "if the change is intended, re-approve the baseline with `loombridge minigame baseline approve`";
  ...
  for (const st of result.states) {
    ...
    if (st.regressed) {
      ...
      incomplete.push({
        ...
        nextAction: `Re-approve the baseline so '${st.state}' has a reference (\`loombridge minigame baseline approve\`).`,
      });
    }
  }
...
export function nextActionFor(
  ...
  if (status === "fail") {
    ...
    if (mustFixObjects.length > 0) {
      ...
      parts.push(
        `fix ${listIds(subjects)} (see the report for the exact screen + check), then re-capture and re-run \`loombridge verify --minigame\``,
      );
    }
    const regressions = ctx.regressions ?? [];
    if (regressions.length > 0) {
      parts.push(
        `review the baseline drift on ${listIds(regressions.map(prettyId))}; if intended, re-approve with \`loombridge minigame baseline approve\` (a regression is not a game defect)`,
      );
    }
    ...
    return parts.length > 0 ? `${parts.join("; ")}.` : "Fix the reported failure(s), then re-capture and re-run `loombridge verify --minigame`.";
  }
  const gated = ctx.outcomeGated ?? [];
  if (gated.length > 0) {
...
function renderReport(report: MinigameVerifyReport, verbose: boolean): void {
  ...
  if (report.baseline) {
    ...
    if (!b.present) {
      line(`\n${ICON.info} Baseline: declared at ${b.ref} but not approved yet; drift not enforced (run \`loombridge minigame baseline approve\`).`);
    } else {
...
export async function runVerifyMinigame(args: VerifyMinigameArgs): Promise<number> {
  ...
  // full build-mode `.loombridge/` tree.
