/**
 * `loombridge mobile-audit` — the mobile-optimization audit report verb (the extraction-shooter dogfood
 * late-polish learnings §9; backlog High #5).
 *
 *   loombridge mobile-audit --input <audit.json> [--out <p>] [--out-text <p>]
...
function usage(): string {
  return [
    "loombridge mobile-audit — advisory mobile-optimization audit over an editor.audit_mobile_assets payload",
    "",
...
async function warnOnClobber(p: string): Promise<void> {
  try {
    await fs.access(p);
    console.warn(`[loombridge mobile-audit] overwriting existing ${p}`);
  } catch {
...
export async function run(argv: string[]): Promise<number> {
  ...
  if ("error" in parsed) {
    console.error(`[loombridge mobile-audit] ${parsed.error}`);
    console.error(usage());
    return 2;
  }
  if (parsed.help) {
  ...
  if (!parsed.input) {
    console.error("[loombridge mobile-audit] --input <path> is required (the saved editor.audit_mobile_assets response)");
    console.error(usage());
    return 2;
  }
  ...
  try {
    ...
    console.error(`[loombridge mobile-audit] cannot read audit payload at ${parsed.input}: ${(e as Error).message}`);
    return 2;
  }
  ...
  if (!kindCheck.ok) {
    console.error(`[loombridge mobile-audit] ${parsed.input}: ${kindCheck.error}`);
    return 2;
  }
