...
/**
 * The six count attributes NUnit writes on a roll-up element.
 *
...
export interface TestsGrade {
   ...
   *
   * NOT a tier. A bare CI XML grades honestly and can reach tier 0; what it cannot do is
   * be quoted, and every door that could quote a green reads THIS rather than re-deriving
   * its own idea of "was this attributable".
   */
  attributed: boolean;
...
export function gradeTestResults(input: TestsGradeInput): TestsGrade {
  const { run, strict, attribution } = input;
