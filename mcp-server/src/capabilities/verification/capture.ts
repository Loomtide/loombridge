...
export { captureKindForSlice, type CaptureKind } from "../../domain/capture-recipes.js";
...
export async function runCapture(args: CaptureArgs, deps: CaptureDeps = defaultDeps): Promise<number> {
  ...
  try {
    if (kind === "tiles") {
      ...
      if (missing.length > 0) {
        console.error(
          `[loombridge capture] missing required tile capture(s): ${missing.join(", ")}. ` +
    ...
    if (!result.pixelPerfectCaptured) {
      console.error(
        "[loombridge capture] note: no PixelPerfectCamera found on the camera — the framing gate's pixel-perfect check will WARN (degraded).",
      );
    }
    return 0;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error(`[loombridge capture] fatal: ${message}`);
    return 1;
  }
}
