...
export function parseTrace(input: unknown): ReplayTrace {
  ...
  return {
    ...
    start: { scene, reset },
    input: { backend },
    // A `viewport` KEY ON DISK IS IGNORED, ON PURPOSE. Traces recorded during the hour the
    // field existed carry the record-time Game view WINDOW size, which is not the frame
    // size and had no honest reader (see `ReplayTrace`). This function rebuilds the trace
    // field by field, so not naming it here is what drops it: those traces keep replaying
    // unchanged rather than being refused for a key nothing consumes.
    segments,
    assertions,
