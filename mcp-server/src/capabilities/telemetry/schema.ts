 ...
 * (`src/capabilities/genre/genre-packs/<genre>/telemetry.json`). Pure path construction — does
...
export function telemetrySchemaPathForGenre(genreId: string): string | null {
  ...
  return path.join(PKG_ROOT, "src", "capabilities", "genre", "genre-packs", genreId, "telemetry.json");
}
