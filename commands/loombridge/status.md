---
description: Report Loombridge slice progress, next step, and actionable warnings without mutating state
allowed-tools: Read, Glob, Grep, Bash(loombridge:*), Bash(node:*), Bash(cat:*)
---

# loombridge status

Report the current Loombridge project state without changing anything. Use this when the
...
3. Treat `Next` as authoritative unless the user explicitly overrides it. `Next` must
   ...
   - `run /loombridge:plan` — scaffold/check design and roadmap.
   - `run /loombridge:build or say continue` — build the current pending/stale slice.
   - `<slice> needs capture/verify evidence; run /loombridge:build or say continue` —
     the agent flow owns deterministic capture/verify details.
   - `say "approve <slice>" or run /loombridge:plan to approve and advance` — a verified
     slice is waiting for human approval. Do not tell the developer to remember `--go`;
     after explicit approval, the agent runs the internal `loombridge plan --go` command.
...
4. Never mutate from this command. Do not run `plan --go`, `build`, `capture`, `verify`,
   or `doneness` as part of `/loombridge:status`. If the developer asks you to continue,
