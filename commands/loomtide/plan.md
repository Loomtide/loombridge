---
description: Scaffold .loomtide/ and establish the design target + acceptance contract for a genre
allowed-tools: Read, Write, Edit, Glob, Grep, AskUserQuestion, WebSearch, WebFetch, Bash(node:*), Bash(loomtide:*), Bash(ls:*), Bash(cat:*)
---
...
**state-driven**: it prints a read-only status echo, then dispatches by `.loomtide/` state —
...
Code (`/loomtide:plan`) and Codex (`scripts/loomtide-plan`) — there is one source of truth,
so the two agents cannot drift.

## Process

1. **Scaffold the contract + check readiness (deterministic).** Run the CLI **zero-param**
   — the developer just runs `loomtide plan` from the project root; the engine is **auto-detected**
   ...
   approved + frozen hero shot exists, then exits non-zero until `.loomtide/ASSET_MANIFEST.json`
   ...
   **ASK THE DEVELOPER WHEN UNCLEAR (agent layer — the CLI never prompts).** The CLI is
   ...
   - If `plan` errors that it **could not detect a game engine project** (exit 2), ask the
     ...
     re-run with `--engine <id>` (e.g. `loomtide plan --engine unity`). Only `unity` is
     supported today.
   - If `plan` errors that it **detected a non-Unity engine** (Loomtide supports Unity only),
...
2. **Read back the state.** `cat .loomtide/STATE.md` to confirm genre/engine/phase.
...
3. **Design Target Phase (§3c — the part that decides "feel").** `plan` is not complete
   ...
   - **Provide:** the user points to a mock / screenshot / HTML. Save the image, then:
     ```bash
     loomtide design set --image <path.png> [--html <path.html>] --mode provided
     ```
   - **Generate (default):** produce an **annotated hero shot built from the genre's asset
     ...
     ```bash
     loomtide design set --image hero-shot.png --html hero-shot.html --mode generated
     ```
   ...
   - **Reference game:** name a game (e.g. Celeste), research its visual + feel conventions,
     ...
     ```bash
     loomtide design set --image hero-shot.png --mode reference-game --reference-game "Celeste"
     ```
   ...
   ```bash
   loomtide design approve --note "approved by <user>"
   ```
   ...
   **3D verticals — the design-target split (`--kind`).** A flat 2D mock cannot faithfully
   ...
   - `rendered-unity-frame` *(default)* — a **real rendered frame** of the assembled scene (a
     ...
     kind eligible for strict hero-shot fidelity + `loomtide doneness`. Existing 2D/platformer
   ...
   1. **Create + approve a composition reference** (the 2D mock as a guide):
      ```bash
      loomtide design set --image style-reference.png --mode generated \
   ...
   4. **Approve/freeze the rendered frame** — this replaces the composition reference:
      ```bash
      loomtide design set --image unity-hero-frame.png --mode provided \
        --kind rendered-unity-frame --approve
      ```
   5. **Only then** run the independent hero-shot fidelity review + `loomtide doneness`.

   `loomtide design status` shows the kind; while it is a `composition-reference` it prints a
...
4. **Record and approve the asset strategy.** Before slices exist, ask the developer which
   ...
   **https://assetstore.loomtide.ai/**; the CLI reads the hosted search API via `--catalog-api`
   ...
   ```bash
   loomtide plan --asset-mode hybrid
   ```

   This writes a **draft** `.loomtide/ASSET_MANIFEST.json` bound to the frozen hero shot. Do
   not hand-edit it to approved. Use the deterministic `loomtide assets` approval helpers so
   ...
   **Offline / test runs only:** swap `--catalog-api <baseUrl>` for `--registry
   ...
   `.loomtide/ASSET_MANIFEST.json` is written. Use `--strict-roles` to disable
   ...
   ```bash
   loomtide assets generated-plan \
     --annotations .loomtide/reports/generated-annotations.json \
     --output .loomtide/reports/generated-asset-plan.json

   loomtide assets generated-apply \
     --annotations .loomtide/reports/generated-annotations.json \
     --exports .loomtide/reports/generated-exports.json \
     --approved-at "<ISO timestamp>"
   ```
...
6. **Build slice by slice.** Tell the user `plan` is complete when the design target is locked,
   `.loomtide/ASSET_MANIFEST.json` is approved, `.loomtide/SLICES.json` exists, and `plan` has
   announced the next unblocked slice. The build
   proceeds **one slice at a time** — review the announced slice, then run `loomtide build` to
   ...
   `loomtide plan --go` yourself, then report the next unblocked slice. If they do not approve,
   ...
   For read-only progress without any scaffold/state mutation, run `loomtide status`.

For local dogfooding from the Loomtide repo before installing the CLI, the equivalent command is
