...
allowed-tools: Read, Write, Edit, Glob, Grep, AskUserQuestion, Bash(node:*), Bash(loomtide:*), Bash(scripts/*), Bash(ls:*), Bash(cat:*), Bash(test:*), mcp__loomtide__*
---

# loomtide e2e

Run a **demo workflow wrapper**, not a third product verb. The public product remains
`loomtide plan` + `loomtide build`; this wrapper is for prepared demos where the brief,
...
2. **Prepare the project.** Use the selected/current Unity project.
   ...
   For an OUTSIDE-repo consumer project, ensure the Loomtide bridge is embedded into its
   `Packages/` first (Tests/ excluded — the embedded `Tests/Editor` asmdef breaks consumer
   compile with CS0246):

   ```bash
   loomtide-embed-bridge --project <unity-project-dir>
...
6. **Build through the real build wrapper.** Use the brief as the intent. `loomtide build`
   ...
   contract's `capturePack`). Prefer efficient Loomtide paths: generated editor builder or
...
7. **Capture required states** into `.loomtide/verify/<state>/`. The bundled platformer
   ...
   **`verify-2d-game`** skill has the full op→args→filename map; in short:
   ...
   `.loomtide/verify/<state>/` subdir. **Also capture one play-mode game frame per state**
   into `.loomtide/verify/<state>/frames/<state>.png` — the consolidated hero-shot review
   (step 8) reads every state's frame, not spawn only. These captures satisfy `doneness`'s
   captureManifest *presence* check; their *quality* is graded by `loomtide verify` on the
...
8. **Run the independent hero-shot review (REQUIRED for a design-targeted demo).** The bundle
   ...
   sub-agent), each given ONLY the bytes of `.loomtide/design/hero-shot.png` + the per-state
   ...
   into ONE consolidated `.loomtide/verify/vlm-review.json` referencing each state's frame by
   its relative path, and stamp the provenance `doneness` reads:

   - `reference.heroShotSha256` = `shasum -a 256 .loomtide/design/hero-shot.png` (MUST equal
