---
description: Run the Tier-1 gates against the .loomtide/ contract and report the enforced verdict
allowed-tools: Read, Glob, Grep, Bash(node:*), Bash(loomtide:*), Bash(cat:*)
---

# loomtide verify

Run the **enforced** Tier-1 gate spine against `.loomtide/ACCEPTANCE.json` and report the
...
1. **Run the gates against the primary state's captures.**
   ...
   ```bash
   # Per-state captures live under .loomtide/verify/<state>/ (see build.md §6).
   ...
   # (published installs: `loomtide verify --root . --inputs .loomtide/verify/spawn`)
   ```

   This reads captured op-output from the named state subdir, writes
   `.loomtide/reports/build-verdict.json` (with the build's `runId` + `producedAt`), updates
   `STATE.md`, and exits non-zero on a Tier-1 `fail` (or on `warn` under `--strict`).

   > **Known limitation (M2 follow-up).** Today `loomtide verify` grades a **single** state
   ...
   > defaults to the bare `.loomtide/verify/` root, which is **not** where the canonical flow
   > writes per-state captures; every gate would degrade to `<gate>.input` WARN.

2. **Report honestly from the verdict.** `cat .loomtide/reports/build-verdict.json`.
   - `status: "pass"` → green; state it plainly.
   - `status: "fail"` → enumerate `failures[]` (gate + expected vs actual). **Do NOT claim
     the build is done.** The next step is `loomtide build` after fixing the reported failures.
   - `status: "warn"` → list `warnings[]`; many warns mean captures are missing from
     `.loomtide/verify/` — note which gates were not measured.
...
4. **A green Tier-1 verdict is NOT "done" for a design-targeted build.** If the project has
   an approved Design Target, `loomtide doneness` additionally requires the verdict to carry
...
```bash
loomtide verify --root . --profile precision \
  --measurements ~/.loomtide/projects/my-game/feel/profile-measurements.json \
...
```bash
loomtide verify --root . --profile precision --setup-capture \
  --player /Player \
...
`~/.loomtide/projects/<id>` from `--id <id>`, or an explicit `--workspace ~/.loomtide/projects/<id>`.
...
fail the profile; Loomtide must not relabel it as a missing or successful short-hop.
...
```bash
loomtide verify --minigame --contract <c>.minigame.json --captures <captures-dir> [--strict]
```
...
- `minigame scan --scene <Assets/...unity>` proposes a DRAFT contract from the scene's visible controls /
  ...
  verdict.
- A demonstration (`loomtide trace record --observe`) establishes the flow / state ordering.
