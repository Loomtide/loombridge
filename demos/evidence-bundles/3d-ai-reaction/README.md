...
`unity-dev-project/shooter-3d-combat-dogfood` fixture. This takes the narrowest AI metric out of the
...
| `generate.mjs` | Assembles the derived artifact FROM the raw transcript: copies the series verbatim, runs `deriveEnemyReactionLatencyMs`, asserts the occluder started active (LOS blocked) and perception precedes reaction. Run: `node demos/evidence-bundles/3d-ai-reaction/generate.mjs`. |
