...
First live **hitscan** capture, taken in the repo-owned `unity-dev-project/shooter-3d-combat-dogfood`
...
| `generate.mjs` | Assembles the derived artifact FROM the raw transcript: copies the series verbatim, runs the production `deriveHitscanImpactLatencyMs` (fire onset `0`), and asserts the fire→hit→damage ordering. Run: `node demos/evidence-bundles/3d-hitscan-weapon/generate.mjs`. |
