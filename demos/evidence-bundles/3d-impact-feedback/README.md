...
`unity-dev-project/shooter-3d-combat-dogfood` fixture. This takes the two combat "juice" metrics
...
| `generate.mjs` | Assembles BOTH derived artifacts FROM the raw transcript: copies samples/series verbatim, runs the production `deriveHitstopMs` / `deriveScreenShakeMag`, asserts the Z-stripped projection refuses, and checks causality. Run: `node demos/evidence-bundles/3d-impact-feedback/generate.mjs`. |
