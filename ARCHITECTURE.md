...
- **Freshness**: the pack script also embeds `package/.loombridge-source-digest`, a digest of the SOURCE bytes it packed (`src/shared/bridge-source-digest.ts`, the one implementation, shelled from bash). The `.sha256` sidecar hashes the tarball itself, so it detects tampering but never staleness: a bundle from an old `prepack` outliving every later commit is exactly what shipped a bridge with missing handlers while `doctor` said healthy. `doctor` grades the bundle (fresh / stale / undigested / unknown-digest-version / unverifiable) and grades the project's tarball bytes against it; `install-bridge` and `update` REFUSE a bundle that does not match this CLI's sources (`--allow-stale-bridge` overrides, loudly; an explicit `--tarball` warns and proceeds).
...
- **`update`**: two phases. First the CLI itself: the install channel is classified (global npm install, local/`npx` copy, frozen runtime, source checkout) and only a global npm install is updated in place, because `npm install -g` would otherwise write to a different copy than the one running; every other channel is detect-and-instruct. A self-update that actually ran ENDS the run and asks for a re-run, since the bridge tarball ships inside the CLI and the new binary must be the one to deliver it. Then the project bridge: hash-checked tarball file-swap + `file:` bump (prune old `.tgz`), backs up `LoombridgeInstall.json`, then runs `doctor`. `--project` defaults to cwd when cwd is a Unity project.
...
- **Workflow**: `minigame init` → `trace record` (play it once) → `minigame capture` (replay drives the pack) → `minigame finalize` (real locators) → `verify --minigame --strict` → `baseline approve`. Live-proven end-to-end on two games (CountTheFruits, ShapeMatch).
...
- **Trace format** (`.loombridge/anchors/traces/<id>.trace.json`): segments of steps — `tap`, `drag`, `world-tap`, `key-tap`, `key-hold`, `key-down`/`key-up`/`wait` (timed-edge keyboard with concurrent holds), `wait-for-visible` — gated by anchors (`ui-visible`, runtime `condition`) with captures tied to anchors. Outcome assertions (captured end-state, `reachedWhenVisible`-gated so an unreached state can't false-pass) close the "flow ok but outcome wrong" hole. Reset tiers: scene-load / game hook / editor relaunch.
- **Recording** (`trace record`): the observer records what the game *responded to* — gesture-time locator resolution (reparenting drags keep the origin path), inert taps dropped + counted, keyboard edges on a timeline, per-step settle timing from the human's own dwell. Traces are green by construction; an empty observation is refused.
- **Replay** (`trace replay`, `replay-all`): drives the trace through the live bridge (UI dispatch for uGUI, simulated Input System keyboard/pointer for gameplay; keepalive holds the input session), captures actuals, compares to the approved baseline with a perceptual YIQ pixel diff (drift is a warning unless `--strict-visual`), and writes a self-contained HTML report (inline images, first-divergence table) plus a fleet roll-up for `replay-all`. Bare `trace replay` (no `--id`) replays the MOST RECENT trace, announced before it drives, with ties broken by name so the choice is reproducible; an explicit `--trace <path>` wins over that search. The WIDENING verbs (`tolerance`, `mask`) deliberately never default: loosening the gate on an existing anchor is the most dangerous change here, and it must name the anchor it touches.
- **Approve** (`trace approve`): human inspects the HTML report, then promotes the run's captures to the baseline. It refuses a run carrying an unreadable capture (a capture gap is a harness fault, never an anchor) and PRESERVES any approved drift tolerance rather than silently re-tightening it. Bare `trace approve` (no `--id`) freezes the MOST RECENT RUN, announced with the number of captures it is about to freeze, so the consent moment states the SIZE of the anchor and not just its name. It searches REPORTS, not traces (the one place it deliberately differs from the `replay` default): the referent is the run you just replayed, and a trace that was recorded but never replayed has nothing to promote.
...
- **Database + API**: Postgres (`assets` table, indexed id/primitive/kind/tags + full `record_json`) behind a company-run **read-only** search API hosted separately from this repo: `/v1/assets/search`, `/v1/assets/:id`, `/v1/catalog/public/...` shards + index. **Its base URL is configuration, not a Loombridge constant**: pass `--catalog-api <baseUrl>` or set `LOOMBRIDGE_ASSET_CATALOG_URL`. This repo deliberately names no deployment host. The base URL is not published at a fixed path, so obtain it from whoever operates the catalog and set it once via `LOOMBRIDGE_ASSET_CATALOG_URL`.
