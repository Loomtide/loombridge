...
namespace UnityBridge.UI
{
    ...
    /// inventory interactions and completes the existing LoomtideAssetPicker
    /// poll handshake when the user confirms.
    /// </summary>
    public class LoomtideAssetBrowser : EditorWindow
    {
        ...
        private const string ApiBaseUrlPrefKey = "loomtide.assetApiBaseUrl";
        ...
        private string _title = "Loomtide Asset Browser";
        ...
        public static void Open(JObject payload)
        {
            ...
            var window = GetWindow<LoomtideAssetBrowser>(utility: false);
            window.titleContent = new GUIContent("Loomtide Asset Browser");
            window.minSize = new Vector2(960f, 620f);
            window.ApplyPayload(payload);

            LoomtideAssetPicker.BeginPendingDecision();
        ...
        [MenuItem("Window/Loomtide/Asset Browser")]
        ...
        private void PumpRequest(UnityWebRequest request, Action onDone)
        {
            ...
            pump = () =>
            {
                ...
                {
                    Debug.LogWarning("[UnityBridge] asset browser request callback failed: " + ex.Message);
                }
            };
        ...
        {
            return Path.Combine(Path.GetTempPath(), "loomtide-browser-cache");
        }
                ...
                try
                {
                    ...
                    if (request.result == UnityWebRequest.Result.Success)
                    {
                        ...
                        if (data != null && data.Length > 0 && !string.IsNullOrEmpty(cachePath))
                        {
                            ...
                            {
                                Debug.LogWarning("[UnityBridge] thumbnail cache write failed: " + ex.Message);
                            }
                        }
                    }
                }
        ...
        private void ApplyPayload(JObject payload)
        {
            ...
            _title = payload.Value<string>("title") ?? "Loomtide Asset Browser";
        ...
        private void BuildToolbar(VisualElement root)
        {
            ...
            brand.Add(brandText);
            brandText.Add(LabelText("Loomtide", 14, Fg0, true));
        ...
        private void BuildStatusBar(VisualElement root)
        {
            ...
            left.Add(dot);
            left.Add(LabelText("Unity Bridge connected - MCP: loomtide - status: " + LoomtideAssetPicker.Status, 11, Fg3, true));
        ...
        {
            private readonly LoomtideAssetBrowser _owner;
            private readonly AssetModel _asset;
            private VisualElement _overlay;
            private VisualElement _imageOverlay;

            public EditorWindowPreview(LoomtideAssetBrowser owner, AssetModel asset)
            {
                _owner = owner;
                _asset = asset;
            }
        ...
        {
            LoomtideAssetPicker.ConfirmSelection(BuildSelectionMap());
        ...
        {
            LoomtideAssetPicker.CancelDecision();
            _suppressDestroyCancel = true;
            Close();
        }
        ...
        private static void RefreshChipRow(VisualElement row, List<string> values, HashSet<string> active, string emptyText)
        {
            ...
            foreach (string value in values)
            {
                var chip = new Button(() =>
                {
                    ...
                    LoomtideAssetBrowser browser = GetWindow<LoomtideAssetBrowser>(utility: false);
                    browser.RefreshAll();
                });
                chip.text = value;
        ...
        private void OnDestroy()
        {
            CancelLiveWork();
            DisposeTextures();
            if (!_suppressDestroyCancel && LoomtideAssetPicker.Status == LoomtideAssetPicker.StatusPending)
                LoomtideAssetPicker.CancelDecision();
        }
    }
