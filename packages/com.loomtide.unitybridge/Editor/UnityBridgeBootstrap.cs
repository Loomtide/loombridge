...
namespace UnityBridge
{
    ...
    public static class UnityBridgeBootstrap
    {
        ...
        static UnityBridgeBootstrap()
        {
            ...
            {
                Debug.Log("[UnityBridge] Skipping bridge startup in secondary/import-worker process");
                return;
            }

            try
            {
                Debug.Log("[UnityBridge] Bootstrap initializing");
                Debug.Log(
                    "[UnityBridge] Transport compatibility target: Unity 6000.3 LTS (primary), 2022.3 LTS (compatibility)");
                ...
                _server.Start();
                Debug.Log(
                    $"[UnityBridge] Bootstrap ready (transport: {_server.ActiveTransportMode}, activePort: {_server.ActivePort})");
                Debug.Log(
                    $"[UnityBridge] Network intent: loopback only via ws://localhost:{_server.ActivePort}, "
            ...
            {
                Debug.LogError($"[UnityBridge] Bootstrap initialization failed: {ex.Message}\n{ex.StackTrace}");
            }
        }
        ...
        {
            Debug.Log("[UnityBridge] Assembly reload detected — stopping server");
            _server?.Stop();
        }
