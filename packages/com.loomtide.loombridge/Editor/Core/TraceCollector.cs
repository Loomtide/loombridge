...
namespace UnityBridge.Core
{
    ...
    public static class TraceCollector
    {
        ...
        private const string ScreenshotDir = "Logs/Loombridge/screenshots";
        ...
        public static void LogInputAction(string action, string backend, string sessionId, string key = null)
        {
            ...
            Debug.Log($"[Loombridge] input action={action} backend={backend ?? "unknown"} session={sessionId ?? ""}{keyFragment}");
        }
        ...
        {
            Debug.LogWarning(
                $"[Loombridge] input failure action={action} backend={backend ?? "unknown"} "
        ...
        public static JObject CaptureErrorScreenshot(string opName)
        {
            ...
            {
                Debug.LogWarning($"[Loombridge] Error screenshot capture failed: {ex.Message}");
                return null;
            }
        }
    }
