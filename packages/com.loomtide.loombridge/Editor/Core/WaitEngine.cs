        ...
        private static void PollAllWaits()
        {
            ...
            for (int i = _activeWaits.Count - 1; i >= 0; i--)
            {
                ...
                if (elapsed > wait.TimeoutMs)
                {
                    ...
                    {
                        Debug.LogError($"[Loombridge] WaitEngine error callback threw: {ex.Message}");
                    }
                    continue;
                }
                ...
                if (wait.Predicate != null)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        {
                            Debug.LogError($"[Loombridge] WaitEngine error callback threw: {callbackEx.Message}");
                        }
                        continue;
                    }
                ...
                {
                    Debug.LogError($"[Loombridge] WaitEngine complete callback threw: {ex.Message}");
                }
            }
