#!/usr/bin/env node
/**
 * sync-loomtide-artifacts — the §3d anti-drift generator (plan §7 #9, §8 M2).
 *
 * The Claude command bodies in `commands/loomtide/*.md` are the single source of
 * truth. The Codex shim (per-command `scripts/loomtide-<name>` wrappers) only
 ...
 *
 *   node scripts/sync-loomtide-artifacts.mjs           # write the artifacts
 *   node scripts/sync-loomtide-artifacts.mjs --check   # CI: fail if drifted
 */
...
const SHIM_SPEC = {
  plan: {
    summary: "loomtide plan",
    argMode: "none",
    prompt:
      "Open and follow commands/loomtide/plan.md. Establish the design target + acceptance contract in .loomtide/.",
    profileComment:
      "`plan` writes .loomtide/ and drives the judgment-heavy Design Target Phase,\n# so: workspace-write + research enabled + medium reasoning.",
    profile: {
  ...
  status: {
    summary: "loomtide status",
    argMode: "none",
    prompt:
      "Open and follow commands/loomtide/status.md. Report read-only Loomtide progress, warnings, and the next command.",
    profileComment:
  ...
  ask: {
    summary: "loomtide ask",
    argMode: "question",
    prompt:
      "Open and follow commands/loomtide/ask.md. Answer this read-only Loomtide workflow question from local project state:",
    profileComment:
      "`ask` is read-only. It runs the deterministic CLI explainer over local\n# .loomtide/ state; no build/capture/verify/approval actions.",
    profile: {
      approval_policy: "on-request",
      sandbox_mode: "read-only",
      model_reasoning_effort: "low",
  ...
  build: {
    summary: "loomtide build",
    argMode: "intent",
    prompt:
      "Open and follow commands/loomtide/build.md. Make the project match the contract for this intent:",
    profileComment:
  ...
  verify: {
    summary: "loomtide verify",
    argMode: "none",
    prompt:
      "Open and follow commands/loomtide/verify.md. Run the Tier-1 gates and report the enforced verdict.",
    profileComment:
  ...
  e2e: {
    summary: "loomtide end-to-end demo workflow",
    argMode: "bundle",
    prompt: "Open and follow commands/loomtide/e2e.md.",
    profileComment:
...
const GENERATED_NOTE = "GENERATED by scripts/sync-loomtide-artifacts.mjs — do not edit by hand.";
...
function promptBlock(name, spec) {
  if (spec.argMode === "intent") {
    return [
      ...
      '# `loomtide-build add coin pickup` → INTENT="add coin pickup").',
      'INTENT="${*:-}"',
  ...
  if (spec.argMode === "bundle") {
    return [
      'BUNDLE="${1:-}"',
      'if [[ -z "$BUNDLE" ]]; then',
      `  PROMPT="${spec.prompt} Ask for the demo bundle path, then run the prepared end-to-end demo workflow through the real Loomtide CLI commands."`,
      "else",
      `  PROMPT="${spec.prompt} Use demo bundle: \${BUNDLE}. Run the prepared end-to-end demo workflow through the real Loomtide CLI commands."`,
      "fi",
    ].join("\n");
  }
...
# (commands/loomtide/${name}.md) — one source of truth, so the two agents cannot drift.
  ...
  if (missing.length) {
    throw new Error(
      `plugin.json declares command(s) with no SHIM_SPEC entry: ${missing.join(", ")}. ` +
        "Add an entry to scripts/sync-loomtide-artifacts.mjs SHIM_SPEC.",
    );
  }
  const artifacts = names.map((name) => ({
    relPath: path.join("scripts", `loomtide-${name}`),
...
/**
 * Generated Codex wrappers are extensionless `scripts/loomtide-<name>` files; the
...
export function staleWrappers(scriptsDir, expectedNames) {
  const expected = new Set(expectedNames.map((n) => `loomtide-${n}`));
  let entries = [];
  try {
  ...
  return entries.filter((f) => /^loomtide-[^.]+$/.test(f) && !expected.has(f));
}
...
function main() {
  ...
  if (check) {
    ...
    if (drifted.length) {
      console.error("[sync-loomtide-artifacts] DRIFT — committed artifacts differ from generated:");
      for (const p of drifted) console.error(`  - ${p}`);
      console.error("Run `node scripts/sync-loomtide-artifacts.mjs` and commit.");
      process.exit(1);
    }
    console.error(`[sync-loomtide-artifacts] OK — ${artifacts.length} artifacts match the generator.`);
  } else {
    ...
    console.error(`[sync-loomtide-artifacts] generated ${artifacts.length} artifacts${stale.length ? `, removed ${stale.length} stale` : ""}.`);
  }
}
