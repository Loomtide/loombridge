#!/usr/bin/env bash
# Installs Loomtide's agent-facing surface (slash commands, skills, AUX harness binaries)
# globally for this user, backed by a FROZEN RUNTIME copied out of the dev repo. After
# install, NO loomtide-* wrapper references the dev repo — they all exec into
# ~/.loomtide/runtime. This is what lets a clean-room validation run be genuinely
# isolated: the dev repo can be relocated/made-absent and the product still works.
#
# THE CORE CLI IS NOT INSTALLED HERE. `loomtide` / `loomtide-mcp` /
# `loomtide-analyze-frames` come from the RELEASE channel:
#
#   curl -fsSL https://get.loomtide.ai | sh      # npm -g @loomtide/cli (install + update)
#
...
#
# Installs:
#   ~/.loomtide/runtime/mcp-server/   frozen copy: dist + src + package.json + node_modules
#   ~/.loomtide/runtime/scripts/      frozen prepare-project-assets.sh + loomtide-embed-bridge.sh
#                                     + loomtide-checkpoint.sh + loomtide-restore.sh (stage harness)
#   ~/.loomtide/runtime/packages/     frozen Unity bridge package (source for the embed step)
#   ~/.loomtide/asset-layer/          profiles + registry + fixtures (source PNG/WAV)
#   ~/.local/bin/loomtide-*           AUX wrappers only (capture/tune/asset-prep/embed-bridge/
#                                     checkpoint/restore/handoff-check) -> the frozen runtime;
#                                     names the released @loomtide/cli provides are excluded
#   ~/.claude/commands/loomtide/      slash commands, repo paths scrubbed
...
# is `loomtide-embed-bridge --project <unity-project>`, installed as a binary below.
#
...
COMMANDS_DIR="${HOME}/.claude/commands/loomtide"
SKILLS_DIR="${HOME}/.claude/skills"
DATA_DIR="${HOME}/.loomtide"
RUNTIME="$DATA_DIR/runtime"
RT_MCP="$RUNTIME/mcp-server"

echo "==> Installing Loomtide from $REPO"
...
# Without these, `loomtide-asset-prep` returns status=fail with an empty asset-cache.
...
# 0c. Vendor the Unity bridge package + freeze the embed script. loomtide-embed-bridge.sh
# resolves its source as `$(dirname $0)/../packages/com.loomtide.unitybridge`, so vendoring
...
echo "  -> $RUNTIME/packages/com.loomtide.unitybridge/ (frozen bridge source for the embed step)"
rm -rf "$RUNTIME/packages/com.loomtide.unitybridge"
mkdir -p "$RUNTIME/packages"
cp -R "$REPO/packages/com.loomtide.unitybridge" "$RUNTIME/packages/com.loomtide.unitybridge"
cp "$REPO/scripts/loomtide-embed-bridge.sh" "$RUNTIME/scripts/loomtide-embed-bridge.sh"
chmod +x "$RUNTIME/scripts/loomtide-embed-bridge.sh"

# 0c-tarball. Pack the bridge TARBALL into the frozen runtime so the DEFAULT
# `loomtide install-bridge` route (file: immutable dependency) can resolve the
...
echo "  -> $RT_MCP/bridge/ (frozen bridge tarball for 'loomtide install-bridge')"
rm -rf "$RT_MCP/bridge"; mkdir -p "$RT_MCP/bridge"
"$REPO/scripts/loomtide-pack-bridge.sh" --out-dir "$RT_MCP/bridge" >/dev/null
cp "$REPO/scripts/loomtide-pack-bridge.sh" "$RUNTIME/scripts/loomtide-pack-bridge.sh"
chmod +x "$RUNTIME/scripts/loomtide-pack-bridge.sh"

# Stage-fixture harness scripts (pure file ops; no runtime/package dependency).
for sh in loomtide-checkpoint.sh loomtide-restore.sh; do
...
# repo. Names the released @loomtide/cli already provides (loomtide, loomtide-mcp,
# loomtide-mcp-server, loomtide-analyze-frames, loomtide-capture-runner,
# loomtide-run-gates) are NEVER created here: ~/.local/bin precedes the npm global bin
...
for stale in loomtide loomtide-mcp loomtide-mcp-server loomtide-analyze-frames loomtide-capture-runner loomtide-run-gates; do
...
echo "  -> $BIN_DIR/loomtide-asset-prep"
cat > "$BIN_DIR/loomtide-asset-prep" <<EOF
#!/usr/bin/env bash
exec "$RUNTIME/scripts/prepare-project-assets.sh" "\$@"
EOF
chmod +x "$BIN_DIR/loomtide-asset-prep"

echo "  -> $BIN_DIR/loomtide-embed-bridge"
cat > "$BIN_DIR/loomtide-embed-bridge" <<EOF
#!/usr/bin/env bash
exec "$RUNTIME/scripts/loomtide-embed-bridge.sh" "\$@"
EOF
chmod +x "$BIN_DIR/loomtide-embed-bridge"

for verb in checkpoint restore; do
  echo "  -> $BIN_DIR/loomtide-$verb"
  cat > "$BIN_DIR/loomtide-$verb" <<EOF
#!/usr/bin/env bash
exec "$RUNTIME/scripts/loomtide-$verb.sh" "\$@"
EOF
  chmod +x "$BIN_DIR/loomtide-$verb"
done
...
# the released @loomtide/cli ships a `loomtide-analyze-frames` bin (see the exclusion list
...
for f in "$REPO/commands/loomtide"/*.md; do
  base="$(basename "$f")"
...
# and not shadowed (a ~/.local/bin `loomtide` here would mean an old shadowing wrapper).
cli_path="$(command -v loomtide || true)"
if [ -z "$cli_path" ]; then
  echo "==> NOTE: no \`loomtide\` on PATH. Install the CLI via the release channel:"
  echo "    curl -fsSL https://get.loomtide.ai | sh"
elif [ "$cli_path" = "$BIN_DIR/loomtide" ]; then
  echo "==> WARNING: $BIN_DIR/loomtide shadows the released CLI — remove it and reinstall:"
  echo "    rm $BIN_DIR/loomtide && curl -fsSL https://get.loomtide.ai | sh"
else
  echo "==> CLI: $cli_path ($(loomtide --version 2>/dev/null || echo 'version unavailable'))"
fi
echo ""
echo "==> Done. Verify with:"
echo "    which loomtide                # expect the npm global bin (e.g. /opt/homebrew/bin/loomtide), NOT $BIN_DIR"
echo "    loomtide --help"
...
echo "    loomtide-embed-bridge --project <unity-project-dir>"
...
echo "    loomtide-checkpoint --project <dir> --stage <construct|level|polish|verify> [--scene <relpath>]"
echo "    loomtide-restore    --project <dir> --stage <name>      # resume from a checkpoint (Unity closed)"
echo "    loomtide verify --stage <name>                          # grade only that stage's gates (diagnostic)"
