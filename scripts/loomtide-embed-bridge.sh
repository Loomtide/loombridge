#!/usr/bin/env bash
# Embed the Loomtide Unity bridge package physically into a CONSUMER Unity
...
# frozen runtime gives the CLI (see scripts/loomtide-install-locally.sh).
#
...
#
# Usage:
#   scripts/loomtide-embed-bridge.sh --project <unity-project-dir> [--package <src>]
#
#   --project   (required) the Unity project to embed into (must contain Packages/ or be creatable)
#   --package   (optional) source package dir (default: this repo's packages/com.loomtide.unitybridge)
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help) usage; exit 0 ;;
    *) echo "loomtide-embed-bridge: unknown argument '$1'" >&2; usage >&2; exit 2 ;;
  esac
done

[ -n "$PROJECT" ] || { echo "loomtide-embed-bridge: --project <unity-project-dir> is required." >&2; exit 2; }
[ -d "$SRC" ] || { echo "loomtide-embed-bridge: source package not found at '$SRC'." >&2; exit 1; }
[ -f "$SRC/package.json" ] || { echo "loomtide-embed-bridge: '$SRC' is not a UPM package (no package.json)." >&2; exit 1; }

PROJECT="$(cd "$PROJECT" 2>/dev/null && pwd || true)"
[ -n "$PROJECT" ] || { echo "loomtide-embed-bridge: --project directory does not exist." >&2; exit 1; }
...
# `loomtide-install-locally.sh`. If you just changed bridge SOURCE in the dev repo,
...
if [ -e "$DEST/Tests" ] || [ -e "$DEST/Tests.meta" ]; then
  echo "loomtide-embed-bridge: FAILED — Tests/ leaked into the embedded package." >&2
  exit 1
fi
[ -d "$DEST/Editor" ] || { echo "loomtide-embed-bridge: FAILED — Editor/ missing from the embed." >&2; exit 1; }
[ -d "$DEST/Runtime" ] || { echo "loomtide-embed-bridge: FAILED — Runtime/ missing from the embed." >&2; exit 1; }
