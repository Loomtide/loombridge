#!/usr/bin/env bash
# Cut a Loomtide CLI release and publish it to GitHub Releases on the PRIVATE
# DISTRIBUTION repo (Loomtide/loombridge — release assets only, no
# source; partners get read access there, never on this monorepo).
#
# Packs @loomtide/cli (its `prepack` bundles the current bridge tarball) and
# uploads the resulting loomtide-cli-<ver>.tgz PLUS scripts/install.sh as release
...
#
# Usage: scripts/loomtide-release.sh [--tag vX.Y.Z] [--notes <str>] [--dry-run] [--deploy-installer]
...
#                        (scripts/deploy-get-loomtide.sh) so the curl|sh channel
#                        serves the current scripts/install.sh
set -euo pipefail

REPO="${LOOMTIDE_REPO:-Loomtide/loombridge}"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) echo "loomtide-release: unknown argument '$1'" >&2; exit 2 ;;
  esac
done
...
echo "==> Packing @loomtide/cli@$ver (prepack bundles the bridge tarball)"
( cd "$DIR/mcp-server" && npm pack >/dev/null )
tgz="$DIR/mcp-server/loomtide-cli-$ver.tgz"
[ -f "$tgz" ] || { echo "loomtide-release: pack did not produce $tgz" >&2; exit 1; }
echo "    $tgz"
...
command -v gh >/dev/null 2>&1 || { echo "loomtide-release: the GitHub CLI (gh) is required to publish." >&2; exit 1; }

notes="${NOTES:-Loomtide CLI ${TAG}. Install or update with one command: \`curl -fsSL https://get.loomtide.ai | sh\`}"
...
# release asset — it only updates when the Vercel `get-loomtide` project is redeployed.
if [ "$DEPLOY_INSTALLER" = 1 ]; then
  echo "==> Refreshing the get.loomtide.ai installer channel"
  bash "$DIR/scripts/deploy-get-loomtide.sh"
else
  echo "!!  get.loomtide.ai still serves the PREVIOUS installer. If scripts/install.sh changed,"
  echo "    refresh it: scripts/deploy-get-loomtide.sh   (or re-run release with --deploy-installer)"
fi
