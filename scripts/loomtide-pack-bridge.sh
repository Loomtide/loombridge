#!/usr/bin/env bash
# Pack the Loomtide Unity bridge package into a versioned UPM tarball (.tgz).
#
...
# as an asset, and what `loomtide install-bridge` installs into a consumer
# project's manifest as a `file:` dependency:
#
#   "com.loomtide.unitybridge": "file:Packages/tarballs/com.loomtide.unitybridge-<ver>.tgz"
#
...
#
# Usage:
#   scripts/loomtide-pack-bridge.sh [--package <src>] [--out-dir <dir>] [--version <x.y.z>]
#
#   --package   source package dir (default: this repo's packages/com.loomtide.unitybridge)
...
#
# Output: <out-dir>/com.loomtide.unitybridge-<version>.tgz
#         plus <out-dir>/com.loomtide.unitybridge-<version>.tgz.sha256

set -euo pipefail

REPO="$(cd "$(dirname "$0")/.." && pwd)"
PKG_NAME="com.loomtide.unitybridge"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h|--help) usage; exit 0 ;;
    *) echo "loomtide-pack-bridge: unknown argument '$1'" >&2; usage >&2; exit 2 ;;
  esac
done

[ -d "$SRC" ] || { echo "loomtide-pack-bridge: source package not found at '$SRC'." >&2; exit 1; }
[ -f "$SRC/package.json" ] || { echo "loomtide-pack-bridge: '$SRC' is not a UPM package (no package.json)." >&2; exit 1; }
...
[ -n "$VERSION" ] || { echo "loomtide-pack-bridge: could not resolve a version." >&2; exit 1; }
...
[ -d "$PKG_ROOT/Editor" ]   || { echo "loomtide-pack-bridge: FAILED — Editor/ missing." >&2; exit 1; }
[ -d "$PKG_ROOT/Runtime" ]  || { echo "loomtide-pack-bridge: FAILED — Runtime/ missing." >&2; exit 1; }
[ -f "$PKG_ROOT/Tests/Editor/$PKG_NAME.tests.asmdef" ] || {
  echo "loomtide-pack-bridge: FAILED — Tests asmdef missing (expected to ship, gated)." >&2; exit 1; }
grep -q 'UNITY_INCLUDE_TESTS' "$PKG_ROOT/Tests/Editor/$PKG_NAME.tests.asmdef" || {
  echo "loomtide-pack-bridge: REFUSING — Tests asmdef lacks the UNITY_INCLUDE_TESTS define constraint." >&2
